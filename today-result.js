...
function build({hour=0,entry,history=[],target=0,anomalies=[]}={}){
 const patients=n(entry?.patients),sales=n(entry?.sales),clinical=entry?.clinical||{},detailCount=[entry?.newPatients,entry?.checkups,entry?.surgeries,clinical.bloodTests,clinical.xrays,clinical.ultrasounds].filter(value=>n(value)>0).length;
 if(!entry||!patients||!sales)return{visible:false,ready:false,reason:"today-data"};
 if(Number(hour)<18&&detailCount<3)return{visible:false,ready:true,reason:"morning"};
 ...
 const anomaly=anomalies.find(item=>item.level&&item.level!=="normal"),improved=baseUnit&&unit>baseUnit;
 const comment=anomaly?`${anomaly.metric}に注意が必要です。明日は原因確認を優先しましょう。`:improved?"客単価が改善しました。明日は新患フォローが優先です。":"今日の実績を確認しました。明日は案内漏れを意識しましょう。";
