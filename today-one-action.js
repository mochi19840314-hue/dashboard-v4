...
const phaseMeta=phase=>phase==="morning"?{shadowTitle:"影武者｜朝の作戦",shadowLead:"朝は先読みだけ。今日の流れを整えます。"}:phase==="midday"?{shadowTitle:"影武者｜診療中の判断",shadowLead:"午前の数字から、午後に変えることは1つだけです。"}:{shadowTitle:"影武者｜今日の振り返り",shadowLead:"今日の結果から、明日に残すことを1つだけ決めます。"};
...
function result(phase,payload){return {phase,title:"今日の一手",...phaseMeta(phase),...payload}}
function build(input={}){try{
 ...
 if(phase==="morning"){
  if(patientEvidence)return result(phase,{action:`予約と来院の流れを確認し、${Math.round(patientEvidence.average)}件前後を無理なく診られる体制を先に整える`,reason:`過去${patientEvidence.sampleSize}回の同曜日平均は${Math.round(patientEvidence.average)}件です。`,evidence:{metric:"patients",...patientEvidence}});
  if(requiredDaily>0)return result(phase,{action:"今日の予約枠と予定を確認し、診療開始前に無理のない流れを整える",reason:`月間目標までを残り${remainingBusinessDays}営業日で割ると、1日平均${yen(requiredDaily)}が目安です。朝は診療内容を指示せず、予定だけ確認します。`,evidence:{metric:"requiredDailySales",value:requiredDaily,remainingBusinessDays}});
  return result(phase,{action:"今日の予約と予定を1回確認する",reason:"比較に必要な同曜日データを蓄積中です。",evidence:{metric:"learning",sampleSize:0}});
 }
 if(phase==="midday"){
  const currentPatients=n(todayEntry.patients),currentSales=n(todayEntry.sales),currentUnit=unit(todayEntry);
  if(patientEvidence&&currentPatients>0){const ratio=(currentPatients/patientEvidence.average-1)*100;if(ratio>=15)return result(phase,{action:"午後は診療負荷を上げすぎず、必要な検査・説明の抜けを防ぐ",reason:`午前時点の来院${currentPatients}件は、同曜日平均${Math.round(patientEvidence.average)}件に対して${pct(ratio)}多い水準です。`,evidence:{metric:"patients",current:currentPatients,changePercent:ratio,...patientEvidence}})}
  if(unitEvidence&&currentUnit>0){const ratio=(currentUnit/unitEvidence.average-1)*100;if(ratio<=-15)return result(phase,{action:"午後は診療内容を見直し、医学的に必要な検査・健診の提案漏れだけ確認する",reason:`午前の客単価${yen(currentUnit)}は同曜日平均${yen(unitEvidence.average)}より${pct(ratio)}低い水準です。`,evidence:{metric:"unitPrice",current:currentUnit,changePercent:ratio,...unitEvidence}})}
  if(requiredDaily>0&&currentSales<requiredDaily*.45)return result(phase,{action:"午後は売上を追うのではなく、未実施の必要検査・健診提案がないか1件ずつ確認する",reason:`午前売上${yen(currentSales)}に対し、今日の目安は${yen(requiredDaily)}です。`,evidence:{metric:"sales",current:currentSales,target:requiredDaily}});
  return result(phase,{action:"午後も今の診療ペースを維持し、記録だけ正確に残す",reason:"午前時点では大きな偏りを検出していません。",evidence:{metric:"stable"}});
 }
 const currentSales=n(todayEntry.sales),currentPatients=n(todayEntry.patients),currentUnit=unit(todayEntry);
 if(salesEvidence&&currentSales>0){const ratio=(currentSales/salesEvidence.average-1)*100;if(Math.abs(ratio)>=15)return result(phase,{action:"今日の差が来院数・客単価・診療構成のどこから出たかメモを1行残す",reason:`今日の売上${yen(currentSales)}は同曜日平均${yen(salesEvidence.average)}より${pct(ratio)}${ratio>=0?"高い":"低い"}結果です。`,evidence:{metric:"sales",current:currentSales,changePercent:ratio,...salesEvidence}})}
 if(unitEvidence&&currentUnit>0){const ratio=(currentUnit/unitEvidence.average-1)*100;if(Math.abs(ratio)>=15)return result(phase,{action:"客単価の差につながった診療内容を1つ記録する",reason:`今日の客単価${yen(currentUnit)}は同曜日平均${yen(unitEvidence.average)}より${pct(ratio)}${ratio>=0?"高い":"低い"}結果です。`,evidence:{metric:"unitPrice",current:currentUnit,changePercent:ratio,...unitEvidence}})}
 return result(phase,{action:"今日の特徴を1行メモして終了する",reason:currentPatients?`本日は${currentPatients}件、売上${yen(currentSales)}でした。大きな偏りは検出していません。`:"本日の実績が未入力です。",evidence:{metric:"summary",patients:currentPatients,sales:currentSales}});
 }catch(error){return result("unknown",{shadowTitle:"影武者｜今日の方針",shadowLead:"安全に確認できる範囲だけ表示します。",action:"今日の診療実績を入力する",reason:"安全に分析できるデータが不足しています。",evidence:{metric:"fallback"}})}}
return {build,phaseMeta};
});
