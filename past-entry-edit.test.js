...
test("past business values, memo, and clinical controls load in the one modal",()=>{const modal=html.match(/<div class="today-entry-modal"[\s\S]*?<\/form>/)[0];assert.match(modal,/data-modal-clinical-output="preventive"/);assert.equal((html.match(/id="todayEntryForm"/g)||[]).length,1);assert.match(app,/entry\?\.memo\?\?entry\?\.note/)});
