...
function renderTodayMission(){
 const container=$("todayMissionList"),comment=$("todayMissionComment"),status=$("todayMissionStatus");if(!container||typeof TodayMission==="undefined")return;let anomalies=[];try{anomalies=typeof BusinessAnomalies!=="undefined"?BusinessAnomalies.detectBusinessAnomalies(data,{today:iso(),hour:new Date().getHours()}):[]}catch(error){console.error(error)}const summary=monthSummary(monthNow()),result=TodayMission.build({entries:data.entries,monthExpense:summary.expense,anomalies});status.textContent=result.ready?`${result.missions.length}件`:`${result.sampleDays}/3日`;
...
function renderTodayWinningStrategy(){
 const content=$("winningStrategyContent"),status=$("winningStrategyStatus");if(!content||typeof TodayWinningStrategy==="undefined")return;let anomalies=[];try{anomalies=typeof BusinessAnomalies!=="undefined"?BusinessAnomalies.detectBusinessAnomalies(data,{today:iso(),hour:new Date().getHours()}):[]}catch(error){console.error(error)}
 const current=monthSummary(monthNow()),months=Object.keys(data.historical||{}).sort().slice(-3).map(month=>data.historical[month]),setting=data.settings[monthNow()]||{},result=TodayWinningStrategy.build({entries:data.entries,configuredProfitRate:setting.profitRate,currentMonthSales:current.sales,currentMonthExpense:current.expense,recentMonths:months,anomalies});status.textContent=result.ready?"AI提案":"データ蓄積中";
