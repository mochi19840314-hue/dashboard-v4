...
function renderExpenseYearOverYear(m,currentFinance){
 ...
 $("expenseYoyRatios").textContent="人件費率・薬品費率：2025年の対応する内訳・売上を確実に特定できないため比較対象外";$("expenseYoyComment").textContent=result.includedMonths.length?`AIコメント：支出は前年同期より${yen(Math.abs(result.cumulative.difference))}${result.cumulative.difference>=0?"増加":"減少"}しています。内訳は比較対象外のため、金額だけで判断せず売上や診療内容との関係も確認してください。`:"AIコメント：入力済み月がないため、前年同期との比較を保留しています。";
}
