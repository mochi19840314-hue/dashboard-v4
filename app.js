...
const CLINICAL_INPUTS={bloodTests:"clinicalBloodTests",xrays:"clinicalXrays",ultrasounds:"clinicalUltrasounds",surgeries:"clinicalSurgeries",newPatients:"clinicalNewPatients",revisits:"clinicalRevisits",preventive:"clinicalPreventive"};
...
function renderClinicalConsistencyWarning(){const warning=$("clinicalConsistencyWarning"),patientsRaw=$("patients").value.trim(),clinical=clinicalFromForm(),classified=clinical.newPatients!==null&&clinical.revisits!==null;if(warning)warning.hidden=!(patientsRaw!==""&&classified&&Number(patientsRaw)!==clinical.newPatients+clinical.revisits)}
...
function clearForm(date=iso()){$("entryDate").value=date;["sales","patients","newPatients","surgeries","checkups","trimmings","secondOpinions"].forEach(id=>$(id).value="");fillClinicalForm({});$("note").value="";$("saveEntry").textContent="保存する";renderTodaySummary()}
function saveEntry(){const date=$("entryDate").value;if(!date)return toast("日付を入力してください");const performanceFields=["sales","patients","newPatients","surgeries","checkups","trimmings","secondOpinions"],hasPerformanceInput=performanceFields.some(id=>$(id).value.trim()!=="");const e={date,sales:num("sales"),patients:num("patients"),newPatients:num("newPatients"),surgeries:num("surgeries"),checkups:num("checkups"),trimmings:num("trimmings"),secondOpinions:num("secondOpinions"),clinical:clinicalFromForm(),weather:data.weatherCache?{condition:data.weatherCache.condition,temperature:data.weatherCache.temperature,rainProbability:data.weatherCache.rainProbability,code:data.weatherCache.code,dailyCondition:data.weatherCache.dailyCondition,dailyCode:data.weatherCache.dailyCode,dailyRainMax:data.weatherCache.dailyRainMax}:null,note:$("note").value.trim()};const i=data.entries.findIndex(x=>x.date===date);if(i>=0)data.entries[i]=e;else data.entries.push(e);data.entries.sort((a,b)=>a.date.localeCompare(b.date));save();const diaryAttempted=date===iso()&&hasPerformanceInput;if(diaryAttempted)saveTodayKagemushaDiary();render();clearForm();if(!diaryAttempted)toast(i>=0?"更新しました":"保存しました")}
