...
function renderBusinessOptimizer(){
 ...
 const items=current.candidates||[];if(!current.ready||!items.length){const remaining=Math.max(0,(current.requiredSamples||BusinessOptimizer.MIN_SAMPLES)-(current.sampleCount||0));status.textContent="学習中";content.innerHTML=`<p class="optimizer-empty">現在学習中です。<br>あと<strong>${remaining}営業日</strong>で最適化を開始します。</p>`;return}
 const primary=items[0],stars="★".repeat(primary.stars);status.textContent="本日の提案";content.innerHTML=`<div class="optimizer-main"><section><small>今日の最優先</small><strong>${escapeHtml(primary.label)}</strong><span class="optimizer-stars stars-${primary.stars}" aria-label="AI評価 ${primary.stars}/5">${stars}</span></section><dl><div><dt>期待利益</dt><dd>＋${yen(primary.expectedProfit)}</dd></div><div><dt>成功率</dt><dd>${primary.successRate}%</dd></div></dl></div><p class="optimizer-reason"><b>理由</b> ${escapeHtml(primary.reason)}</p><ul class="optimizer-actions">${primary.actions.map(action=>`<li>${escapeHtml(action)}</li>`).join("")}</ul><ol class="optimizer-ranking">${items.map(item=>`<li><b>${escapeHtml(item.label)}</b><span>＋${yen(item.expectedProfit)}・${item.successRate}%</span></li>`).join("")}</ol>`;
}
...
function renderStrategyIntelligence(){
 ...
 $("tomorrowStrategy").textContent=`${report.tomorrow.comment} 期待利益 約${yen(report.tomorrow.expectedProfit)}`;
...
function render(){
 const sections=[recent,month,renderMonthlyReport,renderStrategyIntelligence,renderClinicalIntelligence,renderClinicalTrends,years,year,finance,renderClinicSettings,renderBusinessSimulator,storage,renderTodaySummary,renderDailyShadowBrief,renderTodayStrategy,renderBusinessOptimizer,renderSeasonForecast,renderLearningInsight,renderWeeklyInsights,renderSuccessLibrary,renderClinicalLearning,renderBusinessAnomalies,renderDailyReview,renderKagemusha,renderKagemushaDiary,renderPhase1Director,renderManagementInsight];
