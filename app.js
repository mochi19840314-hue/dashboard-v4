...
const base={entries:[],learningHistory:[],weeklyLearningHistory:[],successLibrary:[],clinicalSnapshots:[],successPatterns:[],failurePatterns:[],workloadHistory:[],dailyLearning:{},successScoreHistory:[],businessHealthScore:0,businessHealthHistory:[],strategyMap:{},seasonLearning:{},seasonForecast:{},forecastHistory:[],forecastModel:{},optimizer:{},optimizerHistory:[],dailyRecommendation:{},optimizerScore:{},coachHistory:[],businessSimulator:{changes:{}},simulationHistory:[],goalPlanner:{annualProfit:20000000},improvementModels:{},settings:{},weatherCache:null,meta:{lastUpdated:null},finance:{balance:0,monthlyExpense:0,personnelExpense:0,medicalExpense:0,cardFee:0,loan:0,repayment:0,incomeTarget:0,morikuboOnline:0,royalCanin:0,purina:0},financeByMonth:{},monthlyReports:{},historical:{...HISTORICAL},clinic:{...DEFAULT_CLINIC},memo:""};
...
function renderBusinessOptimizer(){
 ...
 const items=(current.candidates||[]).filter(item=>priority.has(item.key)||item.key==="revisit").slice(0,3);if(!current.ready||!items.length){const remaining=Math.max(0,(current.requiredSamples||BusinessOptimizer.MIN_SAMPLES)-(current.sampleCount||0));status.textContent="学習中";content.innerHTML=`<p class="optimizer-empty">現在学習中です。<br>あと<strong>${remaining}営業日</strong>で最適化を開始します。</p>`;return}
...
function calculateBusinessHealth(){
 const month=monthNow(),summary=monthSummary(month),entries=operatingEntries(data.entries.filter(entry=>entry.date<=iso())),monthEntries=operatingEntries(summary.entries),patients=Number(summary.patients)||0,unitPrice=patients?summary.sales/patients:0;
 const previous=data.entries.filter(entry=>entry.date<`${month}-01`&&Number(entry.patients)>0),normalUnitPrice=previous.length?previous.reduce((sum,entry)=>sum+Number(entry.sales)/Number(entry.patients),0)/previous.length:unitPrice;
 const snapshot=[...(data.clinicalSnapshots||[])].sort((a,b)=>String(a.date).localeCompare(String(b.date))).at(-1)||{},repeat=monthEntries.reduce((sum,entry)=>sum+Math.max(0,Number(entry.repeatPatients??(Number(entry.patients)-Number(entry.newPatients)))||0),0),successPatternMatch=Number(snapshot.successScore)||Number((data.successPatterns||[]).at(-1)?.score)||0,target=Number(data.settings[month]?.target)||MONTHLY_TARGET;
 return BusinessHealthScore.calculate({businessDays:new Set(entries.map(entry=>entry.date)).size,profitRate:summary.sales?(summary.sales-summary.expense)/summary.sales*100:0,salesAchievement:summary.sales/target*100,unitPrice,normalUnitPrice,revisitRate:patients?repeat/patients*100:0,doctorWorkload:Number(snapshot.doctorWorkload)||100,successPatternMatch});
}
function renderBusinessHealth(){
 const result=calculateBusinessHealth(),card=$("businessHealthCard"),content=$("businessHealthContent");card.dataset.tone=result.ready?result.grade.tone:"learning";$("businessHealthGrade").textContent=result.ready?result.grade.label:result.status;
 if(!result.ready){content.innerHTML=`<div class="health-learning"><strong>${result.businessDays} / ${result.requiredDays}営業日</strong><div class="progress"><i style="width:${Math.min(100,result.businessDays/result.requiredDays*100)}%"></i></div><p>${result.businessDays<20?"学習中です。":"精度を高めるため学習を継続しています。"} あと${result.remainingDays}営業日でAI経営指数を開始します。</p></div>`}
 else{const history=BusinessHealthScore.normalizeHistory(data.businessHealthHistory),previous=history.filter(item=>item.date<iso()).at(-1)?.score??result.score,monthStats=BusinessHealthScore.summary(history,monthNow()),delta=result.score-previous,stars="★".repeat(Math.max(1,Math.ceil(result.score/20)))+"☆".repeat(5-Math.max(1,Math.ceil(result.score/20)));content.innerHTML=`<div class="health-hero"><div><strong>${result.score}<small>点</small></strong><span>${stars}</span></div><dl><div><dt>昨日</dt><dd>${delta>=0?"+":""}${delta}</dd></div><div><dt>今月平均</dt><dd>${monthStats.average===null?"—":monthStats.average+"点"}</dd></div></dl></div><p class="health-comment">昨日より${Math.abs(delta)}点${delta>=0?"改善":"変化"}しました。利益率は${Number(result.metrics.profitRate).toFixed(1)}%、Success Patternとの一致率は${Math.round(result.metrics.successPatternMatch)}%でした。</p><section class="health-opportunities"><h3>改善余地</h3>${result.opportunities.map(item=>`<div><span>${"★".repeat(item.stars)}${"☆".repeat(5-item.stars)}</span><b>${item.label}</b></div>`).join("")}</section>`}
 const hasToday=data.entries.some(entry=>entry.date===iso());if(hasToday){const next=BusinessHealthScore.updateHistory(data.businessHealthHistory,result,iso());if(JSON.stringify(next)!==JSON.stringify(data.businessHealthHistory)){data.businessHealthHistory=next;data.businessHealthScore=result.score;save()}}renderBusinessHealthReports(result)
}
function renderBusinessHealthReports(){
 ...
 const report=document.querySelector("#report .report-header")||document.querySelector("#report");if(report){let monthly=$("businessHealthMonthReport");if(!monthly){monthly=document.createElement("article");monthly.id="businessHealthMonthReport";monthly.className="card health-month-report";report.insertAdjacentElement("afterend",monthly)}const monthStats=BusinessHealthScore.summary(data.businessHealthHistory,monthNow());monthly.innerHTML=`<span class="eyebrow">AI経営指数・月間レポート</span><div class="health-summary"><div><small>平均</small><strong>${monthStats.average??"—"}点</strong></div><div><small>最高</small><strong>${monthStats.highest??"—"}点</strong></div><div><small>最低</small><strong>${monthStats.lowest??"—"}点</strong></div><div><small>今月改善</small><strong>${monthStats.improvement===null?"—":`${monthStats.improvement>=0?"+":""}${monthStats.improvement}点`}</strong></div></div>`}}
...
function renderStrategyIntelligence(){
 ...
 data.strategyMap=report.strategyMap;data.seasonLearning=report.seasonLearning;data.forecastModel=report.forecast;
}
...
function buildEntry(date,existing){const performanceFields=["sales","patients","newPatients","surgeries","checkups","trimmings","secondOpinions"],merged={...(existing||{}),date};performanceFields.forEach(id=>merged[id]=entryNumberValue(id,existing));merged.clinical=existing?clinicalFromFormMerged(existing.clinical??{}):clinicalFromForm();merged.weather=existing?.weather??(data.weatherCache?{condition:data.weatherCache.condition,temperature:data.weatherCache.temperature,rainProbability:data.weatherCache.rainProbability,code:data.weatherCache.code,dailyCondition:data.weatherCache.dailyCondition,dailyCode:data.weatherCache.dailyCode,dailyRainMax:data.weatherCache.dailyRainMax}:null);merged.note=$("note").value;merged.memo=merged.note;return merged}
function learnClinicalEntry(entry,transferToCore=false){if(typeof ClinicalLearningEngine==="undefined"||!entry)return;const summary=monthSummary(entry.date.slice(0,7)),days=Math.max(1,operatingEntries(summary.entries).length),result=ClinicalLearningEngine.learnDaily(data,entry,{dailyExpense:summary.expense/days,weather:data.weatherCache,staffCount:data.clinic?.staffCount,benchmarks:{sales:clinicDayInfo(entry.date).target,unitPrice:10000,profitRate:30}});["clinicalSnapshots","successPatterns","failurePatterns","workloadHistory","dailyLearning","successScoreHistory"].forEach(key=>data[key]=result[key]);if(transferToCore&&result.knowledgeCandidates.length)data.successLibrary=KnowledgeCore.update(data.successLibrary,result.knowledgeCandidates)}
...
function edit(date){const e=data.entries.find(x=>x.date===date);if(!e)return;["sales","patients","newPatients","surgeries","checkups","trimmings","secondOpinions"].forEach(id=>$(id).value=e[id]??"");fillClinicalForm(e.clinical??{});$("entryDate").value=e.date;$("note").value=e.note??e.memo??"";setNoteStatus(Boolean((e.note??e.memo??"").length));$("saveEntry").textContent="更新する";preview();switchPage("today");setTimeout(()=>$("entryDate").scrollIntoView({behavior:"smooth",block:"center"}),150)}
...
function render(){
 const sections=[recent,month,renderMonthlyReport,renderStrategyIntelligence,renderClinicalIntelligence,renderClinicalTrends,years,year,finance,renderClinicSettings,renderBusinessSimulator,storage,renderTodaySummary,renderDailyShadowBrief,renderTodayStrategy,renderBusinessHealth,renderBusinessOptimizer,renderSeasonForecast,renderLearningInsight,renderWeeklyInsights,renderSuccessLibrary,renderClinicalLearning,renderBusinessAnomalies,renderDailyReview,renderKagemusha,renderKagemushaDiary,renderPhase1Director,renderManagementInsight];
