...
const base={entries:[],aiFeedback:{},memoLearningHistory:[],memoKnowledge:{keywords:[],sampleCount:0},memoTrends:[],aiRecommendationHistory:[],successRateHistory:[],learningHistory:[],weeklyLearningHistory:[],successLibrary:[],clinicalSnapshots:[],successPatterns:[],failurePatterns:[],workloadHistory:[],efficiencyHistory:[],dailyLearning:{},successScoreHistory:[],businessHealthScore:0,businessHealthHistory:[],meetingBrief:{},meetingHistory:[],strategyMap:{updated:null,themes:[],priorities:[],monthlyHistory:[]},seasonLearning:{},seasonForecast:{},forecastHistory:[],forecastModel:{},optimizer:{},optimizerHistory:[],dailyRecommendation:{},optimizerScore:{},coachHistory:[],businessSimulator:{changes:{}},simulationHistory:[],goalPlanner:{annualProfit:20000000},improvementModels:{},settings:{},weatherCache:null,cardReceiptsByMonth:{},uiState:{analysisExpanded:false},meta:{lastUpdated:null},finance:{balance:0,monthlyExpense:0,personnelExpense:0,medicalExpense:0,cardFee:0,loan:0,repayment:0,incomeTarget:0,morikuboOnline:0,royalCanin:0,purina:0},financeByMonth:{},monthlyReports:{},historical:{...HISTORICAL},clinic:{...DEFAULT_CLINIC},memo:""};
...
function summaryTargetDate(){return iso()}
function todayDashboardData(date=summaryTargetDate()){return TodayDashboardData.build({entries:data.entries,date,clinicalValue:TodayClinicalCounts.value,isOperating:entry=>clinicDayInfo(entry.date).type!=="closed"})}
function renderTodaySummary(){const date=summaryTargetDate(),today=todayDashboardData(date),e=today.entry,patients=Number(e.patients)||0,newPatients=Number(e.newPatients)||0,newPatientRate=patients?Math.max(0,Math.min(100,newPatients/patients*100)):0;$("todaySales").textContent=yen(e.sales);$("todayPatients").textContent=`${patients}件`;$("todayUnit").textContent=yen(patients?e.sales/patients:0);$("todayNew").textContent=`${newPatients}件`;$("todaySummarySales").textContent=yen(e.sales);$("todaySummaryPatients").textContent=`${patients}件`;$("todaySummaryUnit").textContent=yen(patients?e.sales/patients:0);$("todaySummaryNew").textContent=`${newPatients}件`;$("todaySummaryNewRate").textContent=`${Math.round(newPatientRate)}%`;$("todaySummaryPreventive").textContent=`${today.clinical.preventive}件`;$("todaySummaryCheckups").textContent=`${today.clinical.checkups}件`;$("todaySummaryImaging").textContent=`${today.clinical.imaging}件`;$("todaySummaryBloodTests").textContent=`${today.clinical.bloodTests}件`;$("todaySummarySurgeries").textContent=`${today.clinical.surgeries}件`;$("todaySummaryTrimming").textContent=`${today.clinical.trimming}件`;$("todaySummarySecondOpinions").textContent=`${today.clinical.secondOpinions}件`;$("todaySummaryStatus").textContent=date===iso()?(data.entries.includes(e)?"✓ 入力済":"✏ 入力"):`${date.slice(5).replace("-","/")} 編集中`}
function renderTodayWidgets(){
 const date=summaryTargetDate(),today=todayDashboardData(date),entry=today.entry,patients=Number(entry.patients)||0,newPatients=Number(entry.newPatients)||0;
 const health=calculateBusinessHealth(date),score=health.score??health.previewScore,display=businessHealthDisplay(score),workload=health.metrics?.doctorWorkload;
 $("todayHeroSales").textContent=yen(entry.sales);$("todayHeroPatients").textContent=`${patients}件`;$("todayHeroUnit").textContent=yen(patients?Number(entry.sales)/patients:0);$("todayHeroScore").textContent=display.score??"—";$("todayHeroGrade").textContent=display.label??"学習中";
 $("todayWidgetNew").textContent=`${newPatients}件`;$("todayWidgetNewRate").textContent=`${patients?Math.round(newPatients/patients*100):0}%`;$("todayWidgetWorkload").textContent=Number.isFinite(workload)?Math.round(workload):"—";$("todayWidgetWorkloadLabel").textContent=!Number.isFinite(workload)?"算出中":workload>70?"高め":workload<25?"余裕あり":"適正";
 [["Blood","bloodTests"],["Imaging","imaging"],["Preventive","preventive"],["Checkups","checkups"],["Surgeries","surgeries"]].forEach(([id,key])=>$("todayWidget"+id).textContent=today.clinical[key]);
 const summary=DailyAISummary.build({date,entry,entries:today.entries,snapshots:data.clinicalSnapshots,successPatterns:data.successPatterns,failurePatterns:data.failurePatterns}),message=summary.evaluation||summary.judgment?.[0]||"今日の実績を入力すると、最重要ポイントを表示します。",action=summary.actions?.[0]||"まず今日の診療実績を入力";
 $("todayInsightText").textContent=message;$("todayInsightAction").textContent=`→ ${action}`;
 const tags=ClinicalTags.normalize(entry.clinicalTags),context=$("todayContextWidget");context.hidden=!tags.length;$("todayContextTags").innerHTML=tags.map(id=>`<span>${escapeHtml(ClinicalTags.TAGS[id])}</span>`).join("");
 const days=today.businessDays,ratings=Object.values(data.aiFeedback||{}).map(item=>item?.rating).filter(Boolean),points={valid:1,partial:.5,wrong:0},accuracy=ratings.length?Math.round(ratings.reduce((sum,rating)=>sum+(points[rating]??0),0)/ratings.length*100):0;
 $("todayLearningDays").textContent=`${days}営業日`;$("todayLearningSamples").textContent=`院長評価 ${ratings.length}件`;$("todayLearningAccuracy").textContent=ratings.length<10?"学習中":ratings.length<20?`参考精度 ${accuracy}%`:`AI分析精度 ${accuracy}%`;
}
function updateSummaryClinical(metric,delta){const date=summaryTargetDate(),result=TodayClinicalCounts.update(data.entries,date,metric,delta);if(!result)return;save();render();toast(`${date.slice(5).replace("-","/")}の件数を保存しました`)}
...
function businessHealthDisplay(score){const value=Number(score);if(score===null||score===undefined||score===""||!Number.isFinite(value))return {score:null,label:null,text:"診療経営スコア —"};const label=value>=80?"好調":value>=60?"良好":value>=40?"注意":"要改善";return {score:value,label,text:`診療経営スコア ${value}｜${label}`}}
...
function openActivitySummary(date){const row=recentActivityRows().find(item=>item.date===date);if(!row)return;$("activitySummaryModal").dataset.date=row.date;$("activitySummaryDate").textContent=activityDateLabel(row.date);$("activitySummaryContent").innerHTML=`<div class="activity-summary-grid"><section><span>売上</span><strong>${yen(row.sales)}</strong></section><section><span>患者数</span><strong>${row.patients}件</strong></section><section><span>新患</span><strong>${row.newPatients}件</strong></section><section><span>再診</span><strong>${row.revisits}件</strong></section><section><span>画像検査</span><strong>${row.imaging}件</strong></section><section><span>健診</span><strong>${row.checkups}件</strong></section><section><span>利益率</span><strong>${row.profitRate==null?"－":`${row.profitRate.toFixed(1)}%`}</strong></section><section><span>診療経営スコア</span><strong>${businessHealthDisplay(row.health).score==null?"—":`${businessHealthDisplay(row.health).score}｜${businessHealthDisplay(row.health).label}`}</strong></section></div><p class="activity-ai-comment"><strong>AIコメント</strong>${escapeHtml(row.aiComment||"この日のAIコメントはありません。")}</p><p class="activity-memo"><strong>今日のメモ</strong>${row.memo?escapeHtml(row.memo):"記録なし"}</p>`;$("activitySummaryModal").hidden=false;document.body.classList.add("activity-summary-open");$("editActivitySummary").focus()}
function renderRecentActivity(){const list=$("recentActivityList");if(!list)return;const rows=recentActivityRows();list.innerHTML=rows.length?rows.map(row=>`<button class="recent-activity-row" type="button" data-activity-date="${row.date}" aria-label="${activityDateLabel(row.date)}のToday Summaryを表示"><span class="activity-date">${activityDateLabel(row.date)}</span><span class="activity-health-score">${businessHealthDisplay(row.health).text}</span><span>売上 ${yen(row.sales)}</span><span>${row.patients}件</span><span>客単価 ${yen(row.unitPrice)}</span><span>利益率 ${row.profitRate==null?"－":`${row.profitRate.toFixed(1)}%`}</span></button>`).join(""):'<p class="recent-activity-empty">営業日の履歴はまだありません。</p>';list.querySelectorAll("[data-activity-date]").forEach(button=>button.onclick=()=>openActivitySummary(button.dataset.activityDate));renderRecentActivityHealthDebug()}
function setupRecentActivity(){const modal=$("activitySummaryModal");$("closeActivitySummary").onclick=closeActivitySummary;$("editActivitySummary").onclick=()=>{const date=modal.dataset.date;closeActivitySummary();openTodayEntry(date)};modal.onclick=event=>{if(event.target===modal)closeActivitySummary()}}
const TODAY_ENTRY_FIELDS={sales:"todayEntrySales",patients:"todayEntryPatients",newPatients:"todayEntryNewPatients"};
const TODAY_ENTRY_CLINICAL_METRICS=["preventive","checkups","imaging","bloodTests","surgeries","trimming","secondOpinions"];
let todayEntryTargetDate=null,todayEntryClinicalDraft={};
function renderClinicalTagPicker(rootId,selected=[]){const root=$(rootId),chosen=new Set(ClinicalTags.normalize(selected));if(!root)return;root.querySelector(".clinical-tag-groups").innerHTML=ClinicalTags.GROUPS.map(group=>`<div class="clinical-tag-group"><span>${group.label}</span><div class="clinical-tag-chips">${group.tags.map(([id,label])=>`<button type="button" class="clinical-tag-chip" data-clinical-tag="${id}" aria-pressed="${chosen.has(id)}">${label}</button>`).join("")}</div></div>`).join("")}
function selectedClinicalTags(rootId){return [...($(rootId)?.querySelectorAll('[data-clinical-tag][aria-pressed="true"]')||[])].map(button=>button.dataset.clinicalTag)}
function setupClinicalTagPicker(rootId){const root=$(rootId);if(!root)return;root.onclick=event=>{const button=event.target.closest("[data-clinical-tag]");if(button)button.setAttribute("aria-pressed",String(button.getAttribute("aria-pressed")!=="true"))}}
...
function renderTodayEntryRate(){const sales=todayModalNumber("todayEntrySales"),patients=todayModalNumber("todayEntryPatients"),newPatients=todayModalNumber("todayEntryNewPatients"),rate=patients?newPatients/patients*100:0;$("todayEntryUnitPrice").textContent=yen(patients?sales/patients:0);$("todayEntryNewPatientRate").textContent=`${Math.round(rate)}%`}
function renderTodayEntryClinical(){TODAY_ENTRY_CLINICAL_METRICS.forEach(metric=>{const output=document.querySelector(`[data-modal-clinical-output="${metric}"]`);if(output)output.textContent=String(todayEntryClinicalDraft[metric]||0)})}
function openTodayEntry(date=iso()){todayEntryTargetDate=date;const entry=data.entries.find(item=>item.date===date);Object.entries(TODAY_ENTRY_FIELDS).forEach(([key,id])=>$(id).value=entry?.[key]??"");todayEntryClinicalDraft=Object.fromEntries(TODAY_ENTRY_CLINICAL_METRICS.map(metric=>[metric,TodayClinicalCounts.value(entry,metric)]));renderTodayEntryClinical();renderTodayEntryRate();renderClinicalTagPicker("todayModalClinicalTags",entry?.clinicalTags);$("todayEntryModalTitle").textContent=date===iso()?"診療実績を入力・編集":`${activityDateLabel(date)}の診療実績を編集`;$("todayEntryMemo").value=entry?.memo??entry?.note??"";$("todayEntryMemoCount").textContent=`${$("todayEntryMemo").value.length} / 500`;$("todayEntryMemoPanel").hidden=true;$("todayEntryMemoToggle").setAttribute("aria-expanded","false");$("todayEntryError").hidden=true;document.body.classList.add("today-entry-open");openOverlay($("todayEntryModal"));setTimeout(()=>$("todayEntrySales").focus(),0)}
function todayModalNumber(id){const raw=$(id).value.trim();return raw===""?0:Number(raw)}
function applyTodayEntryClinical(entry){entry.clinical={...(entry.clinical||{})};TODAY_ENTRY_CLINICAL_METRICS.forEach(metric=>{const target=todayEntryClinicalDraft[metric]||0,current=TodayClinicalCounts.value(entry,metric);for(let value=current;value<target;value++)TodayClinicalCounts.update([entry],entry.date,metric,1);for(let value=current;value>target;value--)TodayClinicalCounts.update([entry],entry.date,metric,-1)})}
function saveTodayEntry(event){event.preventDefault();const inputs=[...$("todayEntryForm").querySelectorAll('input[type="number"]')],invalid=inputs.some(input=>input.value!==""&&(!Number.isFinite(Number(input.value))||Number(input.value)<0));if(invalid){$("todayEntryError").textContent="0以上の数値を入力してください。";$("todayEntryError").hidden=false;return}const date=todayEntryTargetDate||iso(),index=data.entries.findIndex(item=>item.date===date),existing=index>=0?data.entries[index]:{},patients=todayModalNumber("todayEntryPatients"),newPatients=Math.min(patients,todayModalNumber("todayEntryNewPatients")),entry={...existing,date,clinicalTags:selectedClinicalTags("todayModalClinicalTags"),memo:DailyMemoLearning.clean($("todayEntryMemo").value),sales:todayModalNumber("todayEntrySales"),patients,newPatients};entry.note=entry.memo;if(!entry.weather&&date===iso()&&data.weatherCache)entry.weather={condition:data.weatherCache.condition,temperature:data.weatherCache.temperature,rainProbability:data.weatherCache.rainProbability,code:data.weatherCache.code,dailyCondition:data.weatherCache.dailyCondition,dailyCode:data.weatherCache.dailyCode,dailyRainMax:data.weatherCache.dailyRainMax};applyTodayEntryClinical(entry);if(index>=0)data.entries[index]=entry;else data.entries.push(entry);data.entries.sort((a,b)=>a.date.localeCompare(b.date));learnClinicalEntry(entry,date===iso()&&new Date().getHours()>=18);save();if(date===iso()&&(entry.sales||entry.patients||entry.newPatients))saveTodayKagemushaDiary();closeTodayEntry();render();toast(index>=0?`${activityDateLabel(date)}の診療実績を更新しました`:`${activityDateLabel(date)}の診療実績を保存しました`)}
function setupTodayEntry(){const card=$("todaySummaryCard"),modal=$("todayEntryModal"),sales=$("todayEntrySales"),patients=$("todayEntryPatients"),newPatients=$("todayEntryNewPatients");$("todayHeroEdit").onclick=()=>openTodayEntry(summaryTargetDate());$("todayInsightDetail").onclick=()=>{$("today").classList.add("today-details-open");$("todayDetailToggle").setAttribute("aria-expanded","true");$("aiAnalysisCard").scrollIntoView({behavior:"smooth"})};$("todayDetailToggle").onclick=event=>{const open=$("today").classList.toggle("today-details-open");event.currentTarget.setAttribute("aria-expanded",String(open));event.currentTarget.textContent=open?"診療詳細を閉じる":"診療詳細を見る ＞"};card.onclick=event=>{if(event.target.closest("[data-summary-clinical-step]"))return;openTodayEntry(summaryTargetDate())};card.onkeydown=event=>{if(event.target!==card)return;if(event.key==="Enter"||event.key===" "){event.preventDefault();openTodayEntry(summaryTargetDate())}};$("closeTodayEntry").onclick=closeTodayEntry;$("cancelTodayEntry").onclick=closeTodayEntry;modal.onclick=event=>{if(event.target===modal)closeTodayEntry()};$("todayEntryForm").onsubmit=saveTodayEntry;modal.querySelectorAll("[data-modal-clinical-step]").forEach(button=>button.onclick=()=>{const metric=button.dataset.metric,delta=Number(button.dataset.modalClinicalStep);todayEntryClinicalDraft[metric]=Math.max(0,(todayEntryClinicalDraft[metric]||0)+delta);renderTodayEntryClinical()});$("todayEntryMemoToggle").onclick=()=>{const panel=$("todayEntryMemoPanel"),expanded=panel.hidden;panel.hidden=!expanded;$("todayEntryMemoToggle").setAttribute("aria-expanded",String(expanded));if(expanded)$("todayEntryMemo").focus()};sales.addEventListener("input",renderTodayEntryRate);patients.addEventListener("input",renderTodayEntryRate);newPatients.addEventListener("input",renderTodayEntryRate);$("todayEntryMemo").addEventListener("input",event=>$("todayEntryMemoCount").textContent=`${event.target.value.length} / 500`)}
...
function renderBusinessOptimizer(){
 ...
 const primary=items[0],stars="★".repeat(primary.stars),baseScore=health.score??health.previewScore;status.textContent="90点への改善案";content.innerHTML=`<div class="optimizer-main"><section><small>今日の最優先</small><strong>${escapeHtml(primary.label)}</strong><span class="optimizer-stars stars-${primary.stars}" aria-label="AI評価 ${primary.stars}/5">${stars}</span></section><dl><div><dt>期待利益</dt><dd>＋${yen(primary.expectedProfit)}</dd></div><div><dt>指数予測</dt><dd>${Math.min(100,baseScore+2)}点</dd></div></dl></div><p class="optimizer-reason"><b>理由</b> 診療経営スコアを90点へ近付けます。${escapeHtml(primary.reason)}</p><ul class="optimizer-actions">${primary.actions.map(action=>`<li>${escapeHtml(action)}</li>`).join("")}</ul><ol class="optimizer-ranking">${items.map((item,index)=>`<li><b>${escapeHtml(item.label)}</b><span>あと${index+1}件 ↓ ${Math.min(100,baseScore+2+index*2)}点</span></li>`).join("")}</ol>`;
}

function calculateBusinessHealth(asOf=iso(),rawEntries=data.entries){
 const month=asOf.slice(0,7),normalized=(Array.isArray(rawEntries)?rawEntries:[]).map(entry=>{const date=DateRanges.normalizeEntryDate(entry);return date?{...entry,date}:null}).filter(entry=>entry&&entry.date<=asOf),entries=operatingEntries(normalized),entry=entries.find(item=>item.date===asOf);
 const has=(object,key)=>object!=null&&Object.prototype.hasOwnProperty.call(object,key)&&object[key]!==null&&object[key]!==""&&Number.isFinite(Number(object[key])),value=(object,key)=>has(object,key)?Number(object[key]):null;
 const sales=value(entry,"sales"),patients=value(entry,"patients"),newPatients=value(entry,"newPatients"),prior=entries.filter(item=>item.date<asOf&&Number(item.patients)>0&&Number.isFinite(Number(item.sales))),priorUnits=prior.map(item=>Number(item.sales)/Number(item.patients)).filter(Number.isFinite),normalUnitPrice=priorUnits.length?priorUnits.reduce((a,b)=>a+b,0)/priorUnits.length:10000,priorPatients=prior.map(item=>Number(item.patients)).filter(Number.isFinite),normalPatients=priorPatients.length?priorPatients.reduce((a,b)=>a+b,0)/priorPatients.length:15;
 const clinical=entry?.clinical&&typeof entry.clinical==="object"?entry.clinical:null,rate=count=>patients!==null&&patients>0&&count!==null?count/patients*100:null,imaging=clinical&&has(clinical,"xrays")&&has(clinical,"ultrasounds")?Number(clinical.xrays)+Number(clinical.ultrasounds):null,dailyTarget=(Number(data.settings?.[month]?.target)||MONTHLY_TARGET)/25;
 return BusinessHealthScore.calculate({businessDays:new Set(entries.map(item=>item.date)).size,profitRate:value(entry,"profitRate"),salesAchievement:sales===null?null:sales/dailyTarget*100,patientAchievement:patients===null?null:patients/normalPatients*100,unitPrice:sales!==null&&patients>0?sales/patients:null,normalUnitPrice,newPatientRate:rate(newPatients),revisitRate:patients!==null&&patients>0&&newPatients!==null?(patients-newPatients)/patients*100:null,preventiveRate:rate(clinical&&has(clinical,"preventive")?Number(clinical.preventive):null),checkupRate:rate(has(entry,"checkups")?Number(entry.checkups):null),imagingRate:rate(imaging),bloodTestRate:rate(clinical&&has(clinical,"bloodTests")?Number(clinical.bloodTests):null)});
}
...
function renderClinicalLearning(){
 ...
 const health=calculateBusinessHealth(snapshot.date,data.entries),score=health.score??health.previewScore,display=businessHealthDisplay(score),match=ClinicalLearningEngine.patternMatch(snapshot,data.successPatterns),success=ClinicalLearningEngine.evaluateSuccessDay(snapshot,score);
 const explanation=BusinessHealthScore.explain(health);
 root.innerHTML=`<div class="clinical-learning-score"><small>${escapeHtml(snapshot.date)}</small><em>診療経営スコア</em><strong>${display.score??"—"}</strong><span>${display.label||""}</span></div><div class="clinical-learning-kpis"><section><small>診療負荷</small><strong>${snapshot.doctorWorkload}</strong></section><section><small>成功日</small><strong>${success?"YES":"NO"}</strong></section><section><small>成功パターン一致率</small><strong>${match}%</strong></section></div><section class="clinical-learning-features"><small>学習</small><div>${(learning.features||[]).map(item=>`<span>${escapeHtml(item)}</span>`).join("")||"記録を蓄積中"}</div></section><section class="clinical-learning-reason"><h4>今日の評価理由</h4><p>${explanation.sentences.map(escapeHtml).join("<br>")}</p></section><footer>${escapeHtml(learning.status||"学習中")}・${Number(learning.sampleDays)||0}営業日</footer>`;
}
...
function inspectSuccessLibraryScroll(probe=false){
 const element=$("successLibraryItems");if(!element)return null;const style=getComputedStyle(element),before=element.scrollLeft;
 if(probe&&element.scrollWidth>element.clientWidth)element.scrollLeft=Math.min(100,element.scrollWidth-element.clientWidth);
 const result={element,clientWidth:element.clientWidth,scrollWidth:element.scrollWidth,scrollLeft:element.scrollLeft,overflowX:style.overflowX,touchAction:style.touchAction,scrollable:element.scrollWidth>element.clientWidth,programmaticScrollWorks:!probe||element.scrollWidth<=element.clientWidth||element.scrollLeft!==before};
 if(probe)element.scrollLeft=before;return result;
}
...
function renderStrategyIntelligence(){
 ...
 $("successRanking").innerHTML=(report.scorePatterns.successPatterns.length?report.scorePatterns.successPatterns.map(item=>`<div class="high-score-pattern"><span>${escapeHtml(item.metric)}</span><strong>+${Math.round(item.absDifference)}%</strong></div>`):['<p class="high-score-patterns-empty">比較に必要な営業日データを蓄積中です。</p>']).join("");
 $("managementWarnings").innerHTML=(report.scorePatterns.attentionPatterns.length?report.scorePatterns.attentionPatterns.map(item=>`<div class="strategy-row"><b>⚠</b><span>${escapeHtml(item.text)}</span><strong>−${Math.round(item.absDifference)}%</strong></div>`):(report.warnings.length?report.warnings.map(item=>`<div class="strategy-row"><b>⚠</b><span>${escapeHtml(item.theme)}</span><strong class="strategy-stars">${"★".repeat(item.priority)}${"☆".repeat(5-item.priority)}</strong></div>`):['<p>大きな低下は検出されていません。</p>'])).join("");
 $("strategyMap").innerHTML=(report.scorePatterns.strategies.length?report.scorePatterns.strategies.map(item=>`<div class="strategy-row"><b>改善</b><span>${escapeHtml(item.action)}</span><strong></strong></div>`):Object.entries(report.strategyMap).map(([key,items])=>`<div class="strategy-row"><b>${{strength:"強み",improvement:"改善",growth:"成長"}[key]}</b><span>${escapeHtml(items.join("・")||"学習中")}</span><strong></strong></div>`)).join("");
 ...
 const healthPriority=calculateBusinessHealth().opportunities?.[0]?.label;$("tomorrowStrategy").textContent=`${healthPriority?`診療経営スコアの改善を優先し、${healthPriority}を確認します。 `:""}${report.tomorrow.comment} 期待利益 約${yen(report.tomorrow.expectedProfit)}`;
...
function renderPhase1Director(){
  ...
  const summary=DailyAISummary.build({date:today,entry:todayEntry,entries:data.entries,snapshots:data.clinicalSnapshots,successPatterns:data.successPatterns,failurePatterns:data.failurePatterns,efficiencyHistory:data.efficiencyHistory}),status=summary.status;
...
function learnClinicalEntry(entry,transferToCore=false){if(typeof ClinicalLearningEngine==="undefined"||!entry)return;const summary=monthSummary(entry.date.slice(0,7)),days=Math.max(1,operatingEntries(summary.entries).length),health=calculateBusinessHealth(entry.date,data.entries),businessHealthScore=health.score??health.previewScore,result=ClinicalLearningEngine.learnDaily(data,entry,{dailyExpense:summary.expense/days,weather:entry.weather||data.weatherCache,staffCount:data.clinic?.staffCount,businessHealthScore,benchmarks:{sales:clinicDayInfo(entry.date).target,unitPrice:10000,profitRate:30}});["clinicalSnapshots","successPatterns","failurePatterns","workloadHistory","dailyLearning","successScoreHistory"].forEach(key=>data[key]=result[key]);const review=DailyAISummary.build({date:entry.date,entry,entries:data.entries,snapshots:data.clinicalSnapshots,efficiencyHistory:data.efficiencyHistory}),efficiency=review.meta?.efficiency;if(efficiency)data.efficiencyHistory=[...(Array.isArray(data.efficiencyHistory)?data.efficiencyHistory.filter(item=>item.date!==entry.date):[]),{date:entry.date,...efficiency}].sort((a,b)=>a.date.localeCompare(b.date)).slice(-730);if(transferToCore&&result.knowledgeCandidates.length)data.successLibrary=KnowledgeCore.update(data.successLibrary,result.knowledgeCandidates)}
...
function edit(date){if(!data.entries.some(entry=>entry.date===date))return;switchPage("today");openTodayEntry(date)}
...
function renderDailyAI(){
  const date=iso(),saved=data.entries.find(e=>e.date===date),w=data.weatherCache,day=clinicDayInfo(date);
  const sales=Number(saved?.sales)||0,patients=Number(saved?.patients)||0,newP=Number(saved?.newPatients)||0,second=Number(saved?.secondOpinions)||0;
...
function renderAnnualManagementStatus(){
  ...
  const hasStoredSalesMonth=value=>Object.prototype.hasOwnProperty.call(data.historical||{},value)||data.entries.some(entry=>entry.date.startsWith(`${value}-`))||Object.prototype.hasOwnProperty.call(data.financeByMonth||{},value);
  const recentSales=RecentThreeMonthSales.calculate(month,value=>({sales:monthSummary(value).sales,hasData:hasStoredSalesMonth(value)}));
  ...
  $("annualManagementRecentAverage").textContent=recentSales.average==null?"データ不足":formatMan(recentSales.average);
  const trendLabels={up:"↑",down:"↓",flat:"→ 横ばい",insufficient:"データ不足"},changeLabel=recentSales.change==null?"":`${recentSales.change>=0?"＋":"−"}${Math.abs(recentSales.change).toFixed(1)}% `;
  $("annualManagementRecentTrend").textContent=`前3ヶ月比 ${changeLabel}${trendLabels[recentSales.direction]}`;$("annualManagementRecentTrend").dataset.direction=recentSales.direction;
}
function year(){
  ...
  renderYearChart(rows,y);renderStrategyMap();renderHolidayAnalysis()
}

function holidayMetric(label,target,normal,change,kind){const format=kind==="yen"?v=>yen(v):v=>`${Number(v).toFixed(1)}件`,available=change!==null,symbol=!available?"—":change>5?"↑":change< -5?"↓":"→",signed=!available?"比較データ不足":`${change>=0?"＋":"−"}${Math.abs(change).toFixed(1)}%`;return `<section><span>${label}</span><strong>${target===null?"—":format(target)}</strong><small>通常 ${normal===null?"—":format(normal)}</small><em class="${change>5?"up":change< -5?"down":"flat"}">${symbol} ${signed}</em></section>`}
function renderHolidayAnalysis(){if(typeof HolidayAnalysis==="undefined"||!$("holidayPreset"))return;const start=$("holidayStart").value,end=$("holidayEnd").value,result=HolidayAnalysis.analyze(data.entries,{start,end,isClosed:date=>clinicDayInfo(date).type==="closed"}),labels={high:"高い",normal:"普通",low:"低い",insufficient:"比較データ不足"},examinationNote=$("holidayExaminationNote");examinationNote.hidden=true;examinationNote.innerHTML="";$("holidayValueBadge").textContent=`営業価値：${labels[result.value]||"—"}`;$("holidayValueBadge").dataset.value=result.value||"insufficient";if(!result.valid){$("holidayPeriod").textContent=result.reason;return}$("holidayPeriod").textContent=`対象 ${start.replaceAll("-","/")}〜${end.replaceAll("-","/")}（${result.target.days}営業日）｜通常営業日 ${result.normal.days}日`;$("holidayComparison").innerHTML=holidayMetric("1日平均売上",result.target.averageSales,result.sufficient?result.normal.averageSales:null,result.changes.sales,"yen")+holidayMetric("1日平均患者数",result.target.averagePatients,result.sufficient?result.normal.averagePatients:null,result.changes.patients,"count")+holidayMetric("客単価",result.target.unitPrice,result.sufficient?result.normal.unitPrice:null,result.changes.unitPrice,"yen");const o=result.target.optional,names={newPatients:"新患数",preventive:"予防件数",imaging:"画像検査件数",bloodTests:"血液検査件数",surgeries:"手術件数",trimmings:"トリミング件数"};$("holidayOptional").innerHTML=`<section><span>売上合計</span><strong>${yen(result.target.sales)}</strong></section><section><span>患者数合計</span><strong>${result.target.patients}件</strong></section>`+Object.entries(names).map(([key,label])=>`<section><span>${label}</span><strong>${o[key]===null?"—":`${o[key]}件`}</strong></section>`).join("");$("holidayDecisionTitle").textContent=result.sufficient?`【営業価値：${labels[result.value]}】 ${result.type}`:"比較データ不足";if(!result.sufficient){$("holidayDecisionText").textContent="対象期間の前後4週間に、入力済みの通常営業日が5日以上あると判定できます。";return}const phrase=v=>Math.abs(v)<=5?`ほぼ同等（${Math.abs(v).toFixed(1)}%差）`:`${Math.abs(v).toFixed(1)}%${v<0?"低下":"上昇"}`,advice=result.value==="low"?"来年度は、この期間の一部を休診日にすることを検討できます。":result.value==="high"?"通常診療を維持する価値があります。":"通常診療または一部休診を、勤務体制とあわせて検討できます。";$("holidayDecisionText").textContent=`1営業日平均売上は通常営業日より${phrase(result.changes.sales)}しています。患者数は${phrase(result.changes.patients)}、客単価は${phrase(result.changes.unitPrice)}です。${result.type}と考えられます。→ ${advice}`;const examination=result.examination;if(examination?.dataSufficient&&(examination.comparison.bloodLow||examination.comparison.imagingLow)){const rate=(value)=>`${value.toFixed(1)}%`,row=(label,key,low)=>low?`<span>${label} ${rate(examination.target[key])}　通常 ${rate(examination.normal[key])} <b>↓</b></span>`:"";examinationNote.innerHTML=`<strong>診療内容</strong><div class="holiday-examination-rates">${row("血液検査率","bloodRate",examination.comparison.bloodLow)}${row("画像検査率","imagingRate",examination.comparison.imagingLow)}</div><p>${examination.compositionEffect?"患者数は通常水準ですが、検査実施率が低下しています。売上低下は来院需要より、診療内容・症例構成の影響を受けた可能性があります。必要な検査の提案漏れがなかったか振り返る余地があります。":"検査実施率が通常より低下しています。症例構成による変動もあるため、診療内容とあわせて確認してください。"}</p>`;examinationNote.hidden=false}}
function setupHolidayAnalysis(){if(!$("holidayPreset"))return;const apply=()=>{const p=HolidayAnalysis.preset($("holidayPreset").value,$("yearPicker").value||new Date().getFullYear());if(p){$("holidayStart").value=p.start;$("holidayEnd").value=p.end}renderHolidayAnalysis()};$("holidayPreset").onchange=apply;$("yearPicker").onchange=()=>{year();$("holidayPreset").value==="custom"?renderHolidayAnalysis():apply()};["holidayStart","holidayEnd"].forEach(id=>$(id).onchange=()=>{$("holidayPreset").value="custom";renderHolidayAnalysis()});apply()}
...
function finance(){
  ...
  const hospital=HospitalHouseholdBalance.calculateHospital({clinicalSales:s.clinicalSales,hospitalCashExpense:household.hospitalCashExpense}),hospitalCard=document.querySelector(".hospital-balance-card"),knownExpense=(Number(mf.personnelExpense)||0)+(Number(mf.medicalExpense)||0)+(Number(mf.cardFee)||0)+(Number(snap.repayment)||0),detail=value=>value>0?yen(value):"—";
  $("hospitalClinicalSales").textContent=yen(hospital.clinicalSales);$("hospitalActualExpense").textContent=yen(hospital.hospitalCashExpense);$("hospitalDifference").textContent=signed(hospital.difference);$("hospitalExpenseRate").textContent=hospital.expenseRate==null?"—":`${hospital.expenseRate.toFixed(1)}%`;$("hospitalBreakEven").textContent=hospital.difference<0?`損益分岐まであと ${yen(hospital.breakEvenRemaining)}`:"損益分岐点をクリアしています";$("hospitalMonthStatus").hidden=!(hospital.difference<0&&HospitalHouseholdBalance.isMonthInProgress(m));hospitalCard.classList.toggle("is-positive",hospital.difference>=0);hospitalCard.classList.toggle("is-negative",hospital.difference<0);
...
function renderExpenseYearOverYear(m,currentFinance){
 ...
 const card=document.querySelector(".expense-yoy-card");if(!result.available){card.hidden=true;return}card.hidden=false;
 $("expenseYoyCurrent").textContent=yen(result.cumulative.current);$("expenseYoyPrevious").textContent=yen(result.cumulative.previous);$("expenseYoyDifference").textContent=signed(result.cumulative.difference);$("expenseYoyDifference").className=tone(result.cumulative.difference);$("expenseYoyRate").textContent=formatRate(result.cumulative.rate);$("expenseYoyRate").className=tone(result.cumulative.difference);
 $("expenseYoyPeriod").textContent=result.includedMonths.length?`2026年${result.includedMonths.map(v=>`${v}月`).join("・")}の入力済み月 vs 2025年同月（未入力月は除外）`:"月間支出を入力すると累計比較を表示します。";
...
const AI_FEEDBACK_REASONS={special_case_mix:"患者構成が特殊だった",felt_different:"自分の感覚と違う",closed_day_effect:"休診日の影響",not_exam_proposal:"検査提案の影響ではない",other:"その他"};
function renderAIFeedback(month){const feedback=data.aiFeedback?.[month]||{},detail=$("aiFeedbackDetail");document.querySelectorAll("[data-ai-rating]").forEach(button=>button.setAttribute("aria-pressed",String(button.dataset.aiRating===feedback.rating)));detail.hidden=!(["partial","wrong"].includes(feedback.rating));detail.querySelector(".ai-feedback-reasons").innerHTML=Object.entries(AI_FEEDBACK_REASONS).map(([id,label])=>`<button type="button" data-ai-reason="${id}" aria-pressed="${(feedback.reasons||[]).includes(id)}">${label}</button>`).join("");$("aiFeedbackComment").value=feedback.comment||"";$("aiFeedbackStatus").textContent=feedback.rating?"保存済み":""}
function storeAIFeedback(){const month=reportMonth(),rating=document.querySelector('[data-ai-rating][aria-pressed="true"]')?.dataset.aiRating;if(!rating)return;data.aiFeedback={...(data.aiFeedback||{}),[month]:{month,rating,reasons:[...document.querySelectorAll('[data-ai-reason][aria-pressed="true"]')].map(button=>button.dataset.aiReason),comment:$("aiFeedbackComment").value.trim(),updatedAt:new Date().toISOString()}};save();$("aiFeedbackStatus").textContent="保存済み"}
function setupAIFeedback(){document.querySelector(".ai-feedback-ratings").onclick=event=>{const button=event.target.closest("[data-ai-rating]");if(!button)return;document.querySelectorAll("[data-ai-rating]").forEach(item=>item.setAttribute("aria-pressed",String(item===button)));$("aiFeedbackDetail").hidden=button.dataset.aiRating==="valid";if(button.dataset.aiRating==="valid")$("aiFeedbackDetail").querySelectorAll("[data-ai-reason]").forEach(item=>item.setAttribute("aria-pressed","false"));storeAIFeedback()};$("aiFeedbackDetail").onclick=event=>{const button=event.target.closest("[data-ai-reason]");if(button){button.setAttribute("aria-pressed",String(button.getAttribute("aria-pressed")!=="true"));storeAIFeedback()}};$("aiFeedbackComment").onchange=storeAIFeedback}

function renderSalesVarianceAnalysis(month){
 const content=$("salesVarianceContent"),comparison=$("salesVarianceComparison");if(!content||typeof SalesVarianceAnalysis==="undefined")return;const result=SalesVarianceAnalysis.analyze({month,entries:data.entries});comparison.textContent=result.comparison?.text||"比較対象となる過去データがありません";
 if(result.empty||!result.comparison){content.innerHTML=`<p class="sales-variance-empty">${escapeHtml(result.missing?.[0]||"比較に必要なデータが不足しています。")}</p>`;return}
 const signed=(value,digits=1)=>`${value>=0?"＋":"−"}${Math.abs(value).toLocaleString("ja-JP",{maximumFractionDigits:digits})}`,factorValue=item=>item.unit==="pt"?`${signed(item.delta)}pt`:item.unit==="%"?`${signed(result.baseline[item.key]?item.delta/result.baseline[item.key]*100:0)}%`:`${signed(item.delta)}${item.unit}`;
 const list=(items,empty)=>items.length?items.map((item,index)=>`<li><b>${index+1}</b><span><strong>${escapeHtml(item.name)}</strong><small>売上変動との関連：${item.strength}</small></span><em>${factorValue(item)}</em></li>`).join(""):`<li class="sales-factor-empty">${empty}</li>`,summary=result.summary,impact=result.impacts.filter(x=>Math.abs(x.amount)>=1).map(x=>`<span>${escapeHtml(x.name)} <b>${x.amount>=0?"＋":"−"}${yen(Math.abs(x.amount))}</b></span>`).join("");
 content.innerHTML=`<section class="sales-variance-hero"><span>診療報酬</span><strong>${yen(result.current.sales)}</strong><em class="${result.salesDelta>=0?"up":"down"}">${result.salesDelta>=0?"＋":"−"}${yen(Math.abs(result.salesDelta))}（${result.salesPct==null?"—":`${signed(result.salesPct)}%`}）</em></section>${impact?`<div class="sales-impact-estimates"><small>推定影響額（患者数・客単価・営業日数のみ）</small>${impact}</div>`:""}<div class="sales-factor-columns"><section><h4>⬆ プラス方向の動き</h4><ol>${list(result.positive,"増加方向の比較可能な項目はありません")}</ol></section><section><h4>⬇ 売上を押し下げた要因</h4><ol>${list(result.negative,"減少方向の比較可能な項目はありません")}</ol></section></div><section class="sales-variance-summary"><h4>AI総評</h4><dl><div><dt>経営判断</dt><dd>${escapeHtml(summary.judgment)}</dd></div><div><dt>確認ポイント</dt><dd>${summary.points.map(point=>`<p>${escapeHtml(point)}</p>`).join("")}</dd></div><div><dt>次月の重点</dt><dd><ol>${summary.priorities.map(item=>`<li>${escapeHtml(item)}</li>`).join("")}</ol></dd></div></dl><p>※ 診療項目と売上の因果関係を断定せず、次に確認すべき経営上の論点を示しています。</p>${result.missing.length?`<details><summary>比較できなかった項目（${result.missing.length}）</summary><ul>${result.missing.map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul></details>`:""}</section>`;
}
function renderMonthlyReport(){
  ...
  const memoAnalysis=renderReportMemoAnalysis(s.entries),tagTrends=ClinicalTags.analyze(data.entries,{month:m}),tagInsight=$("reportTagInsight");tagInsight.hidden=!tagTrends.length;tagInsight.innerHTML=tagTrends.length?`<strong>${escapeHtml(tagTrends[0].confidence)}（${tagTrends[0].sampleDays}営業日）</strong>${escapeHtml(ClinicalTags.sentence(tagTrends[0]))}<small>関連傾向であり因果関係を示すものではありません。</small>`:"";renderAIFeedback(m);
  ...
  const condition=progress>=100&&rate>=30?"売上と利益の両面で好調":"改善余地を確認しながら前進",memoContext=memoAnalysis.top.length?` 日々のメモでは${memoAnalysis.top.map(x=>x.label).join("・")}の記録が目立ち、数値変化の背景として注目されます。`:"",tagContext=tagTrends[0]?` ${ClinicalTags.sentence(tagTrends[0])}診療背景の変化が同時に起きていなかったか確認する価値があります。`:"",monthlyTop=KnowledgeCore.rank(data.successLibrary,{limit:5}).map(item=>item.theme).join("・");$("reportSummary").textContent=availability.empty?`${monthLabel(m)}は集計途中です。入力データがそろうまで月間総括を保留します。`:`${monthLabel(m)}は「${condition}」な月です。${driver}が売上を支え、利益率は${pct(rate)}でした。${memoContext}${tagContext}支出では人件費・薬品医療材料費・カード決済手数料の3項目に絞って変化を確認し、来月は売上目標と利益率を両立させることが重点です。${monthlyTop?` Knowledge Core TOP5：${monthlyTop}。`:""}`;
}
...
function setupSwipe(){let sx=0,sy=0,tracking=false;const root=$("pageContainer"),ownsHorizontalScroll=e=>e.target.closest(".success-library-items");root.addEventListener("touchstart",e=>{tracking=false;if(ownsHorizontalScroll(e))return;const t=e.target;if(t.closest("input,textarea,select,button,.table,nav,.recent-activity-card"))return;const p=e.touches[0];sx=p.clientX;sy=p.clientY;tracking=true},{passive:true});root.addEventListener("touchend",e=>{if(ownsHorizontalScroll(e)){tracking=false;return}if(!tracking)return;tracking=false;const p=e.changedTouches[0],dx=p.clientX-sx,dy=p.clientY-sy;if(Math.abs(dx)<60||Math.abs(dx)<Math.abs(dy)*1.25)return;const current=document.querySelector(".page.active")?.id,index=PAGE_IDS.indexOf(current),next=dx<0?index+1:index-1;if(next>=0&&next<PAGE_IDS.length)switchPage(PAGE_IDS[next])},{passive:true});root.addEventListener("touchcancel",()=>{tracking=false},{passive:true})}
...
function renderMorningExecutiveBrief(){
 if(typeof MorningExecutiveBrief==="undefined"||!$("morningBriefContent"))return;const today=iso(),month=monthNow(),summary=monthSummary(month),target=Number(data.settings[month]?.target)||MONTHLY_TARGET,closed=clinicDayInfo(today).type==="closed",hour=new Date().getHours(),learning=KnowledgeCore.rank(data.successLibrary,{limit:1})[0]||null,entry=data.entries.find(item=>item.date===today),scheduledDays=Number(data.settings[month]?.businessDays)||expectedBusinessDays(month),usedDays=new Set(data.entries.filter(item=>item.date?.startsWith(month)&&(Number(item.sales)||Number(item.patients))).map(item=>item.date)).size,remainingBusinessDays=Math.max(0,scheduledDays-usedDays),brief=MorningExecutiveBrief.build({date:today,hour,closed,health:calculateBusinessHealth(),healthHistory:data.businessHealthHistory,entries:data.entries,todayEntry:entry,learning,memoTrends:data.memoTrends,target,sales:summary.sales,remainingBusinessDays});
 ...
 $("morningBriefDate").textContent=new Date(`${today}T00:00:00`).toLocaleDateString("ja-JP",{month:"long",day:"numeric"});
 $("morningBriefContent").innerHTML=`<section><small>今日の目安</small><strong>${brief.guide.amount?yen(brief.guide.amount):"—"}</strong><span>${escapeHtml(brief.guide.note)}</span></section><section><small>今日の注目</small><strong>${escapeHtml(brief.priority.label)}</strong><em>${escapeHtml(brief.priority.reason)}</em></section><section><small>今日の注意</small><strong>${escapeHtml(brief.warning.label)}</strong><span>${escapeHtml(brief.warning.detail)}</span></section><section><small>AIコメント</small><strong>${escapeHtml(brief.comment)}</strong></section>`;
}
