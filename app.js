...
function recentActivityRows(){const entries=(data.entries||[]).map(entry=>{const date=DateRanges.normalizeEntryDate(entry);return date?{...entry,date}:null}).filter(Boolean);return RecentActivity.rows(entries,{closedDates:data.clinic?.closedDates,healthHistory:data.businessHealthHistory,calculateHealth:entry=>{const health=calculateBusinessHealth(entry.date,entries);return health?.score??health?.previewScore??null}})}
