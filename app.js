...
function renderExpenseYearOverYear(m,currentFinance){
 ...
 result.items.forEach(item=>{const target=$(`${item.key}Yoy`);if(!item.comparable){target.textContent="前年データなし";target.className="expense-yoy-inline is-unavailable";return}target.textContent=`今年 ${yen(item.monthly.current)}｜前年同月 ${yen(item.monthly.previous)}｜前年差 ${signed(item.monthly.difference)}｜前年比 ${formatRate(item.monthly.rate)}｜累計 前年同期 ${yen(item.cumulative.previous)} / 今年 ${yen(item.cumulative.current)}`;target.className=`expense-yoy-inline ${tone(item.monthly.difference)}`});
