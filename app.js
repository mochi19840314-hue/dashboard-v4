...
const CLINICAL_INPUTS={bloodTests:"clinicalBloodTests",xrays:"clinicalXrays",ultrasounds:"clinicalUltrasounds",revisits:"clinicalRevisits",preventive:"clinicalPreventive"};
...
function renderClinicalConsistencyWarning(){const warning=$("clinicalConsistencyWarning"),patientsRaw=$("patients").value.trim(),newPatientsRaw=$("newPatients").value.trim(),clinical=clinicalFromForm(),classified=newPatientsRaw!==""&&clinical.revisits!==null;if(warning)warning.hidden=!(patientsRaw!==""&&classified&&Number(patientsRaw)!==Number(newPatientsRaw)+clinical.revisits)}
