...
function openTodayEntry(date=iso()){todayEntryTargetDate=date;const entry=data.entries.find(item=>item.date===date);Object.entries(TODAY_ENTRY_FIELDS).forEach(([key,id])=>$(id).value=entry?.[key]??"");renderClinicalTagPicker("todayModalClinicalTags",entry?.clinicalTags);$("todayEntryModalTitle").textContent=date===iso()?"今日の診療入力":`${activityDateLabel(date)}の診療入力`;$("todayEntryMemo").value=entry?.memo??entry?.note??"";$("todayEntryMemoCount").textContent=`${$("todayEntryMemo").value.length} / 500`;$("todayEntryMemoPanel").hidden=true;$("todayEntryMemoToggle").setAttribute("aria-expanded","false");const patients=Number(entry?.patients)||0,newPatients=Number(entry?.newPatients)||0;$("todayEntryRevisits").value=entry?Math.max(0,patients-newPatients):"";$("todayEntryProfitRate").value=entry?.profitRate??"";$("todayEntryError").hidden=true;document.body.classList.add("today-entry-open");openOverlay($("todayEntryModal"));setTimeout(()=>$("todayEntrySales").focus(),0)}
function todayModalNumber(id){const raw=$(id).value.trim();return raw===""?0:Number(raw)}
function saveTodayEntry(event){event.preventDefault();const inputs=[...$("todayEntryForm").querySelectorAll('input[type="number"]')],invalid=inputs.some(input=>input.value!==""&&(!Number.isFinite(Number(input.value))||Number(input.value)<0));if(invalid){$("todayEntryError").textContent="0以上の数値を入力してください。";$("todayEntryError").hidden=false;return}const date=todayEntryTargetDate||iso(),index=data.entries.findIndex(item=>item.date===date),existing=index>=0?data.entries[index]:{},patients=todayModalNumber("todayEntryPatients"),newPatients=Math.min(patients,todayModalNumber("todayEntryNewPatients")),entry={...existing,date,clinicalTags:selectedClinicalTags("todayModalClinicalTags"),memo:DailyMemoLearning.clean($("todayEntryMemo").value),sales:todayModalNumber("todayEntrySales"),patients,newPatients};entry.note=entry.memo;const profitRaw=$("todayEntryProfitRate").value.trim();if(profitRaw==="")delete entry.profitRate;else entry.profitRate=Number(profitRaw);if(index>=0)data.entries[index]=entry;else data.entries.push(entry);data.entries.sort((a,b)=>a.date.localeCompare(b.date));learnClinicalEntry(entry,date===iso()&&new Date().getHours()>=18);save();closeTodayEntry();render();toast(index>=0?`${activityDateLabel(date)}の診療実績を更新しました`:`${activityDateLabel(date)}の診療実績を保存しました`)}
function setupTodayEntry(){const card=$("todaySummaryCard"),modal=$("todayEntryModal"),patients=$("todayEntryPatients"),newPatients=$("todayEntryNewPatients"),revisits=$("todayEntryRevisits");$("todayHeroEdit").onclick=()=>openTodayEntry(summaryTargetDate());$("todayInsightDetail").onclick=()=>{$("today").classList.add("today-details-open");$("todayDetailToggle").setAttribute("aria-expanded","true");$("aiAnalysisCard").scrollIntoView({behavior:"smooth"})};$("todayDetailToggle").onclick=event=>{const open=$("today").classList.toggle("today-details-open");event.currentTarget.setAttribute("aria-expanded",String(open));event.currentTarget.textContent=open?"診療詳細を閉じる":"診療詳細を見る ＞"};card.onclick=event=>{const step=event.target.closest("[data-summary-clinical-step]");if(step){event.stopPropagation();updateSummaryClinical(step.dataset.metric,Number(step.dataset.summaryClinicalStep));return}openTodayEntry(summaryTargetDate())};card.onkeydown=event=>{if(event.target!==card)return;if(event.key==="Enter"||event.key===" "){event.preventDefault();openTodayEntry(summaryTargetDate())}};$("closeTodayEntry").onclick=closeTodayEntry;$("cancelTodayEntry").onclick=closeTodayEntry;modal.onclick=event=>{if(event.target===modal)closeTodayEntry()};$("todayEntryForm").onsubmit=saveTodayEntry;$("todayEntryMemoToggle").onclick=()=>{const panel=$("todayEntryMemoPanel"),expanded=panel.hidden;panel.hidden=!expanded;$("todayEntryMemoToggle").setAttribute("aria-expanded",String(expanded));if(expanded)$("todayEntryMemo").focus()};const syncRevisits=()=>{if(patients.value===""&&newPatients.value==="")revisits.value="";else revisits.value=String(Math.max(0,todayModalNumber("todayEntryPatients")-todayModalNumber("todayEntryNewPatients")))};patients.addEventListener("input",syncRevisits);newPatients.addEventListener("input",syncRevisits);revisits.addEventListener("input",()=>{if(revisits.value!==""&&patients.value!=="")newPatients.value=String(Math.max(0,todayModalNumber("todayEntryPatients")-todayModalNumber("todayEntryRevisits")))});$("todayEntryMemo").addEventListener("input",event=>$("todayEntryMemoCount").textContent=`${event.target.value.length} / 500`)}
