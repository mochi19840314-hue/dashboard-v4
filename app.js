...
function renderTodaySummary(){const today=iso(),e=data.entries.find(x=>x.date===today)||{sales:0,patients:0,newPatients:0},patients=Number(e.patients)||0,newPatients=Number(e.newPatients)||0,profit=Number(e.profitRate),imaging=Number(e.clinical?.xrays||0)+Number(e.clinical?.ultrasounds||0);$("todaySales").textContent=yen(e.sales);$("todayPatients").textContent=`${patients}件`;$("todayUnit").textContent=yen(patients?e.sales/patients:0);$("todayNew").textContent=`${newPatients}件`;$("todaySummarySales").textContent=yen(e.sales);$("todaySummaryPatients").textContent=`${patients}件`;$("todaySummaryUnit").textContent=yen(patients?e.sales/patients:0);$("todaySummaryProfit").textContent=Number.isFinite(profit)?`${profit.toFixed(1)}%`:"—";$("todaySummaryNew").textContent=`${newPatients}件`;$("todaySummaryRevisit").textContent=patients?`${Math.max(0,(patients-newPatients)/patients*100).toFixed(1)}%`:"—";$("todaySummaryImaging").textContent=`${imaging}件`;$("todaySummaryStatus").textContent=e.date?"✓ 入力済":"✏ 入力"}
const TODAY_ENTRY_FIELDS={sales:"todayEntrySales",patients:"todayEntryPatients",newPatients:"todayEntryNewPatients",checkups:"todayEntryCheckups",surgeries:"todayEntrySurgeries"};
...
function render(){
 const sections=[renderRecommendationOutcome,recent,month,renderMonthlyReport,renderStrategyIntelligence,renderClinicalIntelligence,renderClinicalTrends,updateStrategyMap,years,year,finance,renderClinicSettings,renderBusinessSimulator,storage,renderTodaySummary,renderDailyShadowBrief,renderTodayStrategy,renderBusinessHealth,renderBusinessOptimizer,renderMorningExecutiveBrief,renderSeasonForecast,renderLearningInsight,renderWeeklyInsights,renderSuccessLibrary,renderClinicalLearning,renderBusinessAnomalies,renderDailyReview,renderKagemusha,renderKagemushaDiary,renderPhase1Director,renderManagementInsight];
