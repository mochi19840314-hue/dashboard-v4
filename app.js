...
function renderManagementCompass(result){
 ...
 content.innerHTML=`<div class="compass-priority"><span>${"★".repeat(result.impact)}${"☆".repeat(5-result.impact)}</span><small>今日の最優先</small><h3>${escapeHtml(result.title)}</h3></div><section><h4>Mission</h4><ul>${mission.actions.slice(0,3).map(action=>`<li>${escapeHtml(action)}</li>`).join("")}</ul></section><div class="compass-values"><section><h4>期待売上</h4><strong>${revenue}</strong></section><section><h4>期待利益</h4><strong>${profit}</strong></section><section><h4>成功率</h4><strong>${Math.round(result.successRate)}%</strong></section></div><section class="compass-reason"><h4>理由</h4><p>${escapeHtml(result.reason)}</p></section>${result.next?`<section class="compass-next"><h4>次点</h4><strong>${escapeHtml(result.next.title)}　期待利益 約${yen(result.next.expectedProfit)}</strong></section>`:""}`;
}
function generateTodayStrategy(){
 ...
 return TodayWinningStrategy.generateTodayStrategy({entries:historical,today,readinessStatus:readiness.status,closed:clinicDayInfo(today).type==="closed",currentMonthSales:current.sales,currentMonthExpense:current.expense,recentMonths:months,annualProfitRate,anomalies,learningHistory:data.aiCompassLearning||[]});
}
