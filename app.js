...
function saveEntry(){const date=$("entryDate").value;if(!date)return toast("日付を入力してください");if(!date.startsWith(monthNow()))return toast("過去月は閲覧のみです");const performanceFields=["sales","patients","newPatients","surgeries","checkups","trimmings","secondOpinions"],hasPerformanceInput=performanceFields.some(id=>$(id).value.trim()!=="");const e={date,sales:num("sales"),patients:num("patients"),newPatients:num("newPatients"),surgeries:num("surgeries"),checkups:num("checkups"),trimmings:num("trimmings"),secondOpinions:num("secondOpinions"),weather:data.weatherCache?{condition:data.weatherCache.condition,temperature:data.weatherCache.temperature,rainProbability:data.weatherCache.rainProbability,code:data.weatherCache.code,dailyCondition:data.weatherCache.dailyCondition,dailyCode:data.weatherCache.dailyCode,dailyRainMax:data.weatherCache.dailyRainMax}:null,note:$("note").value.trim()};const i=data.entries.findIndex(x=>x.date===date);if(i>=0)data.entries[i]=e;else data.entries.push(e);data.entries.sort((a,b)=>a.date.localeCompare(b.date));save();const diaryAttempted=date===iso()&&hasPerformanceInput;if(diaryAttempted)saveTodayKagemushaDiary();render();clearForm();if(!diaryAttempted)toast(i>=0?"更新しました":"保存しました")}
...
function renderMonthlyReport(){
  ...
  $("reportPhrase").textContent=`「${model.phrase}」`;$("reportGrade").textContent=model.grade;animateScoreRing($("reportScoreRing"),model.score,"--report-score",$("reportScore"));$("reportScoreStatus").textContent=model.status;$("reportScoreComment").textContent=model.comment;
