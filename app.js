...
const TODAY_ENTRY_CLINICAL_METRICS=["preventive","imaging","bloodTests","surgeries","trimming"];
let todayEntryTargetDate=null,todayEntryClinicalValues={};
...
function renderCashFlowForecast(profitForecast){
  ...
  $("cashFlowBalance").textContent=yen(result.balance);$("cashFlowSalesForecast").textContent=yen(result.receipts.salesForecast);$("cashFlowIncoming").textContent=yen(result.incoming);$("cashFlowOutgoing").textContent=yen(result.outgoing);$("cashFlowForecastBalance").textContent=yen(result.forecastBalance);
  $("cashFlowIncomingDetail").textContent=`カード ${yen(result.receipts.card)}・未回収 ${yen(result.receipts.uncollected)}・その他 ${yen(result.receipts.otherIncome)}`;
  const labels={smbc15:"SMBC 15日",smbcEnd:"SMBC 月末",jcb15:"JCB 15日",jcbEnd:"JCB 月末"},future=new Set(result.cards.futureKeys);
  $("cashFlowReceiptBreakdown").innerHTML=Object.keys(labels).map(key=>`<div>${labels[key]}　${future.has(key)?result.cards.values[key]===null?"未入力":yen(result.cards.values[key]):"入金日経過（対象外）"}</div>`).join("")+`<div>その他　${yen(result.receipts.otherIncome)}</div>`;
