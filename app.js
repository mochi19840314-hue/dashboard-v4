...
function renderMorningExecutiveBrief(){
 ...
 if(brief.mode==="review"&&brief.review){const r=brief.review;$("morningBriefContent").innerHTML=`<section class="brief-review"><h3>本日の結果</h3><span>Business Health <b>${r.healthFrom??"—"}→${r.healthTo??"—"}</b></span><span>画像検査 <b>${r.imaging?"◎":"△"}</b></span><span>健診 <b>${r.checkups?"◎":"△"}</b></span><span>利益率 <b>${r.profitRate?`${r.profitRate>=0?"+":""}${r.profitRate.toFixed(1)}%`:"集計中"}</b></span></section>`;return}
 $("morningBriefContent").innerHTML=`<section><small>今日の総合評価</small><strong>${brief.score==null?"学習中":`🟢 ${escapeHtml(brief.grade)}　${brief.score}点`}</strong><span>前日比 ${delta(brief.previousDelta)} / 今月平均との差 ${delta(brief.monthDelta)}</span></section><section><small>今日の最優先</small><strong>${escapeHtml(brief.priority.label)}</strong><span>${brief.priority.expectedProfit?`期待利益 +${yen(brief.priority.expectedProfit)}`:"経営基盤を整備"}${brief.priority.successRate!=null?` / 成功率 ${brief.priority.successRate}%`:""}</span><em><b>理由</b> ${escapeHtml(brief.priority.reason)}</em></section><section><small>今週の注意点</small><strong>${escapeHtml(brief.warning.label)}</strong><span>${escapeHtml(brief.warning.detail)}</span></section><section><small>今月目標</small><strong>達成率 ${brief.goal.achievement}%</strong><span>残り ${Math.ceil(brief.goal.remaining/10000)}万円</span></section><section><small>AIコメント</small><strong>${escapeHtml(brief.comment)}</strong></section>`;
}
