...
function renderBusinessHealthReports(){
 ...
 renderBusinessHealthMonthReport(reportMonth())}
function businessHealthMonthStats(month){
 const history=BusinessHealthScore.normalizeHistory(data.businessHealthHistory);let rows=history.filter(item=>item.date.startsWith(month));const savedScore=Number(data.businessHealthScore);
 if(month===monthNow()&&Number.isFinite(savedScore)&&savedScore>0&&!rows.some(item=>item.date===iso()))rows=[...rows,{date:iso(),score:Math.max(0,Math.min(100,Math.round(savedScore)))}];
 return BusinessHealthScore.summary(rows);
}
function renderBusinessHealthMonthReport(month=reportMonth()){
 const monthly=$("businessHealthMonthReport");if(!monthly)return;const stats=businessHealthMonthStats(month),value=number=>number==null?"—":`${number}点`,improvement=stats.improvement==null?"—":`${stats.improvement>=0?"+":""}${stats.improvement}点`;
 monthly.innerHTML=`<span class="eyebrow">AI経営指数・月間レポート</span><div class="health-summary"><div><small>平均</small><strong>${value(stats.average)}</strong></div><div><small>最高</small><strong>${value(stats.highest)}</strong></div><div><small>最低</small><strong>${value(stats.lowest)}</strong></div><div><small>今月改善</small><strong>${improvement}</strong></div></div>`;
}
...
function renderMonthlyReport(){
  ...
  renderBusinessHealthMonthReport(m);
}
