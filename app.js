...
function recentActivityRows(){const entries=(data.entries||[]).map(entry=>{const date=DateRanges.normalizeEntryDate(entry);return date?{...entry,date}:null}).filter(Boolean);return RecentActivity.rows(entries,{closedDates:data.clinic?.closedDates,healthHistory:data.businessHealthHistory,calculateHealth:entry=>{try{const health=calculateBusinessHealth(entry.date,entries);return health?.score??health?.previewScore??null}catch(error){console.warn("[Recent Activity Health Failed]",{date:entry.date,error});return null}}})}
...
function calculateBusinessHealth(asOf=iso(),rawEntries=data.entries){
 ...
 const snapshot=[...(data.clinicalSnapshots||[])].filter(item=>String(item.date)<=asOf).sort((a,b)=>String(a.date).localeCompare(String(b.date))).at(-1)||{},repeat=monthEntries.reduce((total,entry)=>total+Math.max(0,Number(entry.repeatPatients??(Number(entry.patients)-Number(entry.newPatients)))||0),0),pattern=[...(data.successPatterns||[])].filter(item=>!item?.date||String(item.date)<=asOf).at(-1),successPatternMatch=Number(snapshot.successScore)||Number(pattern?.score)||0,target=Number(data.settings[month]?.target)||MONTHLY_TARGET;
