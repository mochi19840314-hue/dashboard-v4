...
function renderTodayMission(){
 const container=$("todayMissionList"),comment=$("todayMissionComment"),status=$("todayMissionStatus");if(!container||typeof TodayMission==="undefined")return;const today=iso(),historical=data.entries.filter(entry=>entry.date<today),readiness=ClinicalIntelligence.getClinicalAnalysisReadiness(historical,{closedDates:data.clinic?.closedDates||[]});let anomalies=[];try{anomalies=typeof BusinessAnomalies!=="undefined"?BusinessAnomalies.detectBusinessAnomalies(data,{today,hour:new Date().getHours()}):[]}catch(error){console.error(error)}const summary=monthSummary(monthNow()),result=TodayMission.build({entries:historical,today,readinessStatus:readiness.status,closed:clinicDayInfo(today).type==="closed",monthExpense:summary.expense,anomalies});status.textContent=result.closed?"休診日":result.ready?`${result.missions.length}件`:`${readiness.sampleDays}日`;
 if(result.closed){container.innerHTML='<p class="today-mission-empty">本日は休診日です。<br>必要な確認だけ行いましょう。</p>';comment.hidden=true;return}
 if(!result.ready){container.innerHTML='<p class="today-mission-empty">診療データ蓄積中です。<br>十分なデータが集まると<br>今日のミッションを提案します。</p>';comment.hidden=true;return}const medals=["🥇","🥈","🥉"],stars=["★★★★★","★★★★☆","★★★☆☆"];container.innerHTML=result.missions.map((mission,index)=>`<article class="mission-card"><div class="mission-title"><h3>${medals[index]} ${escapeHtml(mission.title)}</h3><span aria-label="優先度5段階中${5-index}">${stars[index]}</span></div><section><h4>今日やること</h4>${mission.actions.map(action=>`<p>□ ${escapeHtml(action)}</p>`).join("")}</section><section class="mission-effects"><h4>期待効果</h4>${mission.effects.map(effect=>`<p>＋${escapeHtml(effect)}</p>`).join("")}</section></article>`).join("");if(result.comment){comment.innerHTML=`<h3>💡 AIから一言</h3><p>今日は「<strong>${escapeHtml(result.comment.quote)}</strong>」を最優先にしてください。</p><dl><div><dt>理由</dt><dd>${escapeHtml(result.comment.reason)}からです。</dd></div><div><dt>期待できる効果</dt><dd>${escapeHtml(result.comment.effect)}</dd></div></dl>`;comment.hidden=false}else comment.hidden=true;
}
function renderTodayWinningStrategy(){
 const content=$("winningStrategyContent"),status=$("winningStrategyStatus");if(!content||typeof TodayWinningStrategy==="undefined")return;const today=iso(),historical=data.entries.filter(entry=>entry.date<today),readiness=ClinicalIntelligence.getClinicalAnalysisReadiness(historical,{closedDates:data.clinic?.closedDates||[]});let anomalies=[];try{anomalies=typeof BusinessAnomalies!=="undefined"?BusinessAnomalies.detectBusinessAnomalies(data,{today,hour:new Date().getHours()}):[]}catch(error){console.error(error)}
 const current=monthSummary(monthNow()),months=Object.keys(data.historical||{}).sort().slice(-3).map(month=>data.historical[month]),setting=data.settings[monthNow()]||{},result=TodayWinningStrategy.build({entries:historical,today,readinessStatus:readiness.status,closed:clinicDayInfo(today).type==="closed",configuredProfitRate:setting.profitRate,currentMonthSales:current.sales,currentMonthExpense:current.expense,recentMonths:months,anomalies});status.textContent=result.closed?"休診日":result.ready?"AI提案":"データ蓄積中";
 if(result.closed){content.innerHTML='<p class="winning-strategy-empty">本日は休診日です。<br>必要な確認だけ行いましょう。</p>';return}
 if(!result.ready){content.innerHTML='<p class="winning-strategy-empty">診療データ蓄積中です。<br><br>十分なデータが集まると<br>今日最も効果の高い行動を<br>AIが提案します。</p>';return}
 const stars="★".repeat(result.impact)+"☆".repeat(5-result.impact),revenue=result.expectedRevenue?`約${yen(result.expectedRevenue)}`:"直接売上なし",profit=result.expectedRevenue?`約${yen(result.estimatedProfit)}`:"—";content.innerHTML=`<div class="winning-action"><span>🎯</span><h3>${escapeHtml(result.title)}</h3></div><section class="winning-impact"><h4>経営インパクト</h4><strong aria-label="5段階中${result.impact}">${stars}</strong></section><section><h4>期待効果</h4><div class="winning-tags">${result.effects.map(effect=>`<span>＋${escapeHtml(effect)}</span>`).join("")}</div></section><div class="winning-values"><section><h4>期待売上</h4><strong>${revenue}</strong></section><section><h4>推定利益 <small>予測値</small></h4><strong>${profit}</strong></section></div><p class="winning-formula">期待売上 × ${escapeHtml(result.profitRateSource)}利益率（${pct(result.profitRate)}）＝ 推定利益</p><section class="winning-reason"><h4>理由</h4><p>${escapeHtml(result.reason)}</p></section><p class="winning-disclaimer">※ 推定利益は入力データに基づく予測値であり、成果を保証するものではありません。</p>`;
}
