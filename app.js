...
function renderStrategyIntelligence(){
 ...
 $("successRanking").innerHTML=(report.scorePatterns.successPatterns.length?report.scorePatterns.successPatterns.map((item,index)=>`<div class="strategy-row"><b>${index+1}</b><span>${escapeHtml(item.text)}<br><small>${report.scorePatterns.reliability}</small></span><strong>＋${Math.round(item.absDifference)}%</strong></div>`):(report.successRanking.length?report.successRanking.map((item,index)=>`<div class="strategy-row"><b>${index+1}</b><span>${escapeHtml(item.theme)}<br><small>信頼度 ${item.confidence}/5</small></span><strong>${item.profitImprovement>=0?"＋":""}${item.profitImprovement}%</strong></div>`):['<p>成功パターンを学習中です。</p>'])).join("");
