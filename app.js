...
function renderKagemusha(){
 ...
 const image=$("kagemushaImage"),emoji=$("kagemushaEmoji");if(emoji){emoji.textContent="🥷";emoji.hidden=false}
 if(image){const requestedMood=mood;image.hidden=true;image.dataset.mood=requestedMood;image.onload=()=>{if(image.dataset.mood!==requestedMood)return;image.hidden=false;if(emoji)emoji.hidden=true};image.onerror=()=>{if(image.dataset.mood!==requestedMood)return;image.hidden=true;image.removeAttribute("src");if(emoji)emoji.hidden=false};image.src=`assets/kagemusha-${requestedMood}.png`}
