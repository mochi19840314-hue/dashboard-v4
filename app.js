...
function renderSalesVarianceAnalysis(month){
 ...
 content.innerHTML=`<section class="sales-variance-hero"><span>診療報酬</span><strong>${yen(result.current.sales)}</strong><em class="${result.salesDelta>=0?"up":"down"}">${result.salesDelta>=0?"＋":"−"}${yen(Math.abs(result.salesDelta))}（${result.salesPct==null?"—":`${signed(result.salesPct)}%`}）</em></section>${impact?`<div class="sales-impact-estimates"><small>推定影響額（患者数・客単価・営業日数のみ）</small>${impact}</div>`:""}<div class="sales-factor-columns"><section><h4>⬆ 売上を支えた要因</h4><ol>${list(result.positive,"増加方向の比較可能な項目はありません")}</ol></section><section><h4>⬇ 売上を押し下げた要因</h4><ol>${list(result.negative,"減少方向の比較可能な項目はありません")}</ol></section></div><section class="sales-variance-summary"><h4>AI総評</h4><dl><div><dt>結論</dt><dd>${escapeHtml(summary.conclusion)}</dd></div><div><dt>主な増加要因</dt><dd>${escapeHtml(summary.increase)}</dd></div><div><dt>主な減少要因</dt><dd>${escapeHtml(summary.decrease)}</dd></div><div><dt>AI判断</dt><dd>${escapeHtml(summary.judgment)}</dd></div><div><dt>来月見るポイント</dt><dd>${escapeHtml(summary.next)}</dd></div></dl>${result.seasonality?`<p class="sales-seasonality"><strong>季節性</strong> ${escapeHtml(result.seasonality)}</p>`:""}<p>※ 診療項目は金額換算せず、売上との関連傾向として表示しています。因果関係を断定するものではありません。</p>${result.missing.length?`<details><summary>比較できなかった項目（${result.missing.length}）</summary><ul>${result.missing.map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul></details>`:""}</section>`;
}
