...
function renderExpenseYearOverYear(m,currentFinance){
 ...
 const card=document.querySelector(".expense-yoy-card"),inline=$("monthlyExpenseYoy");if(!result.available){inline.textContent="前年比較は2026年が対象です";inline.className="expense-yoy-inline is-unavailable";card.hidden=true;return}card.hidden=false;
 inline.textContent=`前年同月 ${yen(result.monthly.previous)}｜前年差 ${signed(result.monthly.difference)}｜前年比 ${formatRate(result.monthly.rate)}`;inline.className=`expense-yoy-inline ${tone(result.monthly.difference)}`;
 result.items.forEach(item=>{const target=$(`${item.key}Yoy`);if(!item.comparable){target.textContent="前年データなし";target.className="expense-yoy-inline is-unavailable";return}const c=item.cumulative,coverage=`${c.enteredCount}/${c.expectedCount}か月入力済み`,warning=c.complete?"":`⚠️ ${coverage}（欠損：${c.missingMonths.map(value=>`${value}月`).join("・")}）`;target.textContent=`今年 ${yen(item.monthly.current)}｜前年同月 ${yen(item.monthly.previous)}｜前年差 ${signed(item.monthly.difference)}｜前年比 ${formatRate(item.monthly.rate)}｜今年累計 ${yen(c.current)}（${coverage}）｜前年同月累計 ${yen(c.previous)}｜累計前年差 ${signed(c.difference)}｜累計前年比 ${formatRate(c.rate)}${warning?`｜${warning}`:""}`;target.className=`expense-yoy-inline ${warning?"is-incomplete":tone(item.monthly.difference)}`});
 ...
 $("expenseYoyRatios").textContent="人件費は2025年「給与総合」、薬品・医療材料費は「薬品合計」と比較。カード決済手数料は前年データなし。";$("expenseYoyComment").textContent=result.includedMonths.length?`AIコメント：支出は前年同期より${yen(Math.abs(result.cumulative.difference))}${result.cumulative.difference>=0?"増加":"減少"}しています。比較可能な内訳と売上・診療内容の関係も確認してください。`:"AIコメント：入力済み月がないため、総支出の前年同期比較を保留しています。";
}
