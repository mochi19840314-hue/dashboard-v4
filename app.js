...
function renderMorningExecutiveBrief(){
 ...
 $("morningBriefContent").innerHTML=`<section><small>今日の最優先</small><strong>${escapeHtml(brief.priority.label)}</strong><em>${escapeHtml(brief.priority.reason)}</em></section><section><small>診療経営スコア</small><strong>${businessHealthDisplay(brief.score).score==null?"—":`${businessHealthDisplay(brief.score).score}点｜${businessHealthDisplay(brief.score).label}`}</strong><span>前日比 ${delta(brief.previousDelta)}</span></section><section><small>今日の期待利益</small><strong>${brief.priority.expectedProfit?`+${yen(brief.priority.expectedProfit)}`:"算出中"}</strong><span>${brief.priority.successRate!=null?`成功率 ${brief.priority.successRate}%`:"データを学習中"}</span></section><section><small>今日のリスク</small><strong>${escapeHtml(brief.warning.label)}</strong><span>${escapeHtml(brief.warning.detail)}</span></section><section><small>AIコメント</small><strong>${escapeHtml(brief.comment)}</strong></section>`;
}
