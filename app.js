...
function summaryTargetDate(){return $("entryDate")?.value||iso()}
function renderTodaySummary(){const date=summaryTargetDate(),e=data.entries.find(x=>x.date===date)||{sales:0,patients:0,newPatients:0,clinical:{}},patients=Number(e.patients)||0,newPatients=Number(e.newPatients)||0,newPatientRate=patients?Math.max(0,Math.min(100,newPatients/patients*100)):0,profit=Number(e.profitRate);$("todaySales").textContent=yen(e.sales);$("todayPatients").textContent=`${patients}件`;$("todayUnit").textContent=yen(patients?e.sales/patients:0);$("todayNew").textContent=`${newPatients}件`;$("todaySummarySales").textContent=yen(e.sales);$("todaySummaryPatients").textContent=`${patients}件`;$("todaySummaryUnit").textContent=yen(patients?e.sales/patients:0);$("todaySummaryProfit").textContent=Number.isFinite(profit)?`${profit.toFixed(1)}%`:"—";$("todaySummaryNew").textContent=`${newPatients}件`;$("todaySummaryNewRate").textContent=`${Math.round(newPatientRate)}%`;$("todaySummaryPreventive").textContent=`${TodayClinicalCounts.value(e,"preventive")}件`;$("todaySummaryCheckups").textContent=`${TodayClinicalCounts.value(e,"checkups")}件`;$("todaySummaryImaging").textContent=`${TodayClinicalCounts.value(e,"imaging")}件`;$("todaySummaryBloodTests").textContent=`${TodayClinicalCounts.value(e,"bloodTests")}件`;$("todaySummarySurgeries").textContent=`${TodayClinicalCounts.value(e,"surgeries")}件`;$("todaySummaryTrimming").textContent=`${TodayClinicalCounts.value(e,"trimming")}件`;$("todaySummarySecondOpinions").textContent=`${TodayClinicalCounts.value(e,"secondOpinions")}件`;$("todaySummaryStatus").textContent=date===iso()?(e.date?"✓ 入力済":"✏ 入力"):`${date.slice(5).replace("-","/")} 編集中`}
...
function updateSummaryClinical(metric,delta){const date=summaryTargetDate(),result=TodayClinicalCounts.update(data.entries,date,metric,delta);if(!result)return;save();if($("entryDate")?.value===date)fillClinicalForm(result.entry.clinical);render();toast(`${date.slice(5).replace("-","/")}の件数を保存しました`)}
...
function renderTodayEntryRate(){const patients=todayModalNumber("todayEntryPatients"),newPatients=todayModalNumber("todayEntryNewPatients"),rate=patients?newPatients/patients*100:0;$("todayEntryNewPatientRate").textContent=`${Math.round(rate)}%`}
