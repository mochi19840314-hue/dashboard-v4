...
const PAYMENT_DAY_ITEMS={salary:"人件費",rent:"家賃",medical:"薬品・医療材料費",card:"カード決済関連支出",lease:"リース・返済",tax:"税金",other:"その他"};
function paymentDaysForMonth(month){
 const current=data.financeByMonth?.[month]?.paymentDays;if(current&&typeof current==="object")return current;
 const previous=Object.keys(data.financeByMonth||{}).filter(key=>key<month&&data.financeByMonth[key]?.paymentDays).sort().pop();
 return previous?data.financeByMonth[previous].paymentDays:(data.finance?.paymentDays||{});
}
function selectedPaymentDays(){return Object.fromEntries(Object.keys(PAYMENT_DAY_ITEMS).map(key=>[key,$(`paymentDay-${key}`)?.value?Number($(`paymentDay-${key}`).value):null]))}
function renderCashFlowForecast(profitForecast){
  ...
  const days=paymentDaysForMonth(m),payments={salary,rent,medical,card,lease,tax,other};Object.keys(payments).forEach(key=>payments[`${key}Day`]=days[key]);
  const result=CashFlowForecast.calculate({balance:value("balance"),currentSales:s.sales,projectedSales:profitForecast.projectedSales,uncollected:value("uncollectedIncome"),otherIncome:value("otherIncome"),cardReceipts,today,payments,previousBalance:(prev.balance||Object.prototype.hasOwnProperty.call(data.financeByMonth,monthShift(m,-1)))?prev.balance:null,currentProfit:profitForecast.currentProfit,previousMedicalExpense:Number(data.financeByMonth[monthShift(m,-1)]?.medicalExpense)||0});
  $("cashFlowBalance").textContent=yen(result.balance);$("cashFlowIncoming").textContent=yen(result.incoming);$("cashFlowOutgoing").textContent=yen(result.outgoing);$("cashFlowMinimumBalance").textContent=yen(result.minimumBalance);$("cashFlowForecastBalance").textContent=yen(result.forecastBalance);
  ...
  $("cashFlowMinimumBalance").nextElementSibling.textContent=result.hasUnscheduledPayments?"概算・未設定の支払日があります":"資金繰り上の最低地点";
...
function finance(){
  ...
  const paymentDays=paymentDaysForMonth(m),amounts={salary:Number(mf.personnelExpense??f.personnelExpense)||0,rent:Number(mf.rentExpense??f.rentExpense)||0,medical:Number(mf.medicalExpense??f.medicalExpense)||0,card:Number(mf.cardFee??f.cardFee)||0,lease:Number(mf.leaseExpense??snap.repayment)||0,tax:Number(mf.taxExpense??f.taxExpense)||0};amounts.other=Number(mf.otherExpense??f.otherExpense)||Math.max(0,expense-Object.values(amounts).reduce((sum,value)=>sum+value,0));
  $("paymentDayFields").innerHTML=Object.entries(PAYMENT_DAY_ITEMS).map(([key,label])=>`<label class="payment-day-row"><span>${label}<small>${yen(amounts[key])}</small></span><select id="paymentDay-${key}" aria-label="${label}の支払日"><option value="">未設定</option>${Array.from({length:31},(_,index)=>`<option value="${index+1}"${Number(paymentDays[key])===index+1?" selected":""}>${index+1}日</option>`).join("")}</select></label>`).join("");
...
function saveFinance(){const m=$("monthPicker").value||monthNow(),household={hospitalCashExpense:num("hospitalCashExpense"),householdExpense:num("householdExpense")},paymentDays=selectedPaymentDays();if(m!==monthNow()){data.financeByMonth[m]={...(data.financeByMonth[m]||{}),...household,paymentDays};save();finance();return toast(`${m}の病院＋家計・支払日を保存しました`)}const ec={morikuboOnline:num("morikuboOnline"),royalCanin:num("royalCanin"),purina:num("purina")},expenses={personnelExpense:num("personnelExpense"),medicalExpense:num("medicalExpense"),cardFee:num("cardFee")},entered={monthlyExpense:$("monthlyExpense").value.trim()!=="",personnelExpense:$("personnelExpense").value.trim()!=="",medicalExpense:$("medicalExpense").value.trim()!=="",cardFee:$("cardFee").value.trim()!==""};data.finance={balance:num("balance"),monthlyExpense:num("monthlyExpense"),loan:num("loan"),repayment:num("repayment"),incomeTarget:num("incomeTarget"),...ec,...expenses,paymentDays};data.financeByMonth[m]={...(data.financeByMonth[m]||{}),monthlyExpense:num("monthlyExpense"),balance:num("balance"),loan:num("loan"),repayment:num("repayment"),...ec,...expenses,...household,entered,paymentDays};save();finance();month();renderMonthlyReport();year();toast(`${m}の財務・EC売上・病院＋家計 収支を保存しました`)}
