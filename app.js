...
function saveTodayKagemushaDiary(showToast=true){const values=currentKagemushaData(),entries=loadKagemushaDiaries(),index=entries.findIndex(entry=>entry.date===values.date),next=generateKagemushaDiaryEntry(values,index>=0?entries[index]:null);if(index>=0)entries[index]=next;else entries.push(next);saveKagemushaDiaries(entries);renderKagemushaDiary();if(showToast)toast("影武者日誌を更新しました")}
...
function saveEntry(){const date=$("entryDate").value;if(!date)return toast("日付を入力してください");const performanceFields=["sales","patients","newPatients","surgeries","checkups","trimmings","secondOpinions"],hasPerformanceInput=performanceFields.some(id=>$(id).value.trim()!=="");const e={date,sales:num("sales"),patients:num("patients"),newPatients:num("newPatients"),surgeries:num("surgeries"),checkups:num("checkups"),trimmings:num("trimmings"),secondOpinions:num("secondOpinions"),weather:data.weatherCache?{condition:data.weatherCache.condition,temperature:data.weatherCache.temperature,rainProbability:data.weatherCache.rainProbability,code:data.weatherCache.code,dailyCondition:data.weatherCache.dailyCondition,dailyCode:data.weatherCache.dailyCode,dailyRainMax:data.weatherCache.dailyRainMax}:null,note:$("note").value.trim()};const i=data.entries.findIndex(x=>x.date===date);if(i>=0)data.entries[i]=e;else data.entries.push(e);data.entries.sort((a,b)=>a.date.localeCompare(b.date));save();if(date===iso()&&hasPerformanceInput)saveTodayKagemushaDiary(false);render();clearForm();toast(i>=0?"更新しました":"保存しました")}
...
function openClinicalTrends(){
  ...
  $("clinicalTrendModal").hidden=false;$("closeClinicalTrends").focus();
}
function closeClinicalTrends(){if($("clinicalTrendModal"))$("clinicalTrendModal").hidden=true}
