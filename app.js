...
function renderTodaySummary(){const date=summaryTargetDate(),e=data.entries.find(x=>x.date===date)||{sales:0,patients:0,newPatients:0,clinical:{}},patients=Number(e.patients)||0,newPatients=Number(e.newPatients)||0,newPatientRate=patients?Math.max(0,Math.min(100,newPatients/patients*100)):0;$("todaySales").textContent=yen(e.sales);$("todayPatients").textContent=`${patients}件`;$("todayUnit").textContent=yen(patients?e.sales/patients:0);$("todayNew").textContent=`${newPatients}件`;$("todaySummarySales").textContent=yen(e.sales);$("todaySummaryPatients").textContent=`${patients}件`;$("todaySummaryUnit").textContent=yen(patients?e.sales/patients:0);$("todaySummaryNew").textContent=`${newPatients}件`;$("todaySummaryNewRate").textContent=`${Math.round(newPatientRate)}%`;$("todaySummaryPreventive").textContent=`${TodayClinicalCounts.value(e,"preventive")}件`;$("todaySummaryCheckups").textContent=`${TodayClinicalCounts.value(e,"checkups")}件`;$("todaySummaryImaging").textContent=`${TodayClinicalCounts.value(e,"imaging")}件`;$("todaySummaryBloodTests").textContent=`${TodayClinicalCounts.value(e,"bloodTests")}件`;$("todaySummarySurgeries").textContent=`${TodayClinicalCounts.value(e,"surgeries")}件`;$("todaySummaryTrimming").textContent=`${TodayClinicalCounts.value(e,"trimming")}件`;$("todaySummarySecondOpinions").textContent=`${TodayClinicalCounts.value(e,"secondOpinions")}件`;$("todaySummaryStatus").textContent=date===iso()?(e.date?"✓ 入力済":"✏ 入力"):`${date.slice(5).replace("-","/")} 編集中`}
function renderTodayWidgets(){
 const date=summaryTargetDate(),entry=data.entries.find(item=>item.date===date)||{sales:0,patients:0,newPatients:0,clinical:{}},patients=Number(entry.patients)||0,newPatients=Number(entry.newPatients)||0;
 ...
 [["BloodTests","bloodTests"],["Imaging","imaging"],["Preventive","preventive"],["Checkups","checkups"],["Surgeries","surgeries"]].forEach(([id,key])=>$("todayWidget"+id).textContent=TodayClinicalCounts.value(entry,key));
 const summary=DailyAISummary.build({date,entry,entries:data.entries,snapshots:data.clinicalSnapshots,successPatterns:data.successPatterns,failurePatterns:data.failurePatterns}),message=summary.evaluation||summary.judgment?.[0]||"今日の実績を入力すると、最重要ポイントを表示します。",action=summary.actions?.[0]||"まず今日の診療実績を入力";
 ...
 const days=operatingEntries(data.entries.filter(item=>item.date<=date)).length,ratings=Object.values(data.aiFeedback||{}).map(item=>item?.rating).filter(Boolean),points={valid:1,partial:.5,wrong:0},accuracy=ratings.length?Math.round(ratings.reduce((sum,rating)=>sum+(points[rating]??0),0)/ratings.length*100):0;
