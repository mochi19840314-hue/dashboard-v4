...
function renderTodayMission(result){
 const container=$("todayMissionList"),comment=$("todayMissionComment"),status=$("todayMissionStatus");if(!container||!result)return;status.textContent=result.closed?"休診日":result.ready?`${result.missions.length}件`:`${result.sampleDays}日`;
...
function renderTodayWinningStrategy(result){
 const content=$("winningStrategyContent"),status=$("winningStrategyStatus");if(!content||!result)return;status.textContent=result.closed?"休診日":result.ready?"AI提案":"データ蓄積中";
 ...
 const stars="★".repeat(result.impact)+"☆".repeat(5-result.impact),revenue=result.expectedSales?`約${yen(result.expectedSales)}`:"直接売上なし",profit=result.expectedSales?`約${yen(result.expectedProfit)}`:"—";content.innerHTML=`<div class="winning-action"><span>🎯</span><h3>${escapeHtml(result.title)}</h3></div><section class="winning-impact"><h4>経営インパクト</h4><strong aria-label="5段階中${result.impact}">${stars}</strong></section><section><h4>期待効果</h4><div class="winning-tags">${result.effects.map(effect=>`<span>＋${escapeHtml(effect)}</span>`).join("")}</div></section><div class="winning-values"><section><h4>期待売上</h4><strong>${revenue}</strong></section><section><h4>期待利益</h4><strong>${profit}</strong></section><section><h4>利益率</h4><strong>${pct(result.profitRate)}</strong></section></div><p class="winning-formula">${escapeHtml(result.profitRateDescription)}</p><section class="winning-reason"><h4>理由</h4><p>${escapeHtml(result.reason)}</p></section><p class="winning-disclaimer">※ 推定利益は入力データに基づく予測値であり、成果を保証するものではありません。</p>`;
}
...
function renderTodayStrategy(){const result=generateTodayStrategy();renderTodayWinningStrategy(result);renderTodayMission(result)}
...
function render(){
 const sections=[recent,month,renderMonthlyReport,renderClinicalIntelligence,renderClinicalTrends,years,year,finance,renderClinicSettings,storage,renderTodaySummary,renderDailyShadowBrief,renderTodayStrategy,renderBusinessAnomalies,renderDailyReview,renderKagemusha,renderKagemushaDiary,renderPhase1Director,renderManagementInsight];
