...
function renderClinicalLearning(){
 ...
 root.innerHTML=`<div class="clinical-learning-score"><small>${escapeHtml(snapshot.date)}</small><em>診療経営スコア</em><strong>${display.score??"—"}</strong><span>${display.label||""}</span></div><div class="clinical-learning-kpis"><section><small>診療負荷</small><strong>${snapshot.doctorWorkload}</strong></section><section><small>成功日</small><strong>${success?"YES":"NO"}</strong></section><section><small>成功パターン一致率</small><strong>${match}%</strong></section></div><section class="clinical-learning-features"><small>学習</small><div>${(learning.features||[]).map(item=>`<span>${escapeHtml(item)}</span>`).join("")||"記録を蓄積中"}</div></section><p>${escapeHtml(learning.comment||"")}</p><footer>${escapeHtml(learning.status||"学習中")}・${Number(learning.sampleDays)||0}営業日</footer>`;
}
