...
function renderBusinessHealthReports(){
 ...
 const report=document.querySelector("#report .report-header")||document.querySelector("#report");if(report){let monthly=$("businessHealthMonthReport");if(!monthly){monthly=document.createElement("article");monthly.id="businessHealthMonthReport";monthly.className="card health-month-report";report.insertAdjacentElement("afterend",monthly)}const reportHistory=BusinessHealthScore.reportHistory(data.businessHealthHistory,data.entries),monthStats=BusinessHealthScore.summary(reportHistory,monthNow());monthly.innerHTML=`<span class="eyebrow">AI経営指数・月間レポート</span><div class="health-summary"><div><small>平均</small><strong>${monthStats.average??"—"}点</strong></div><div><small>最高</small><strong>${monthStats.highest??"—"}点</strong></div><div><small>最低</small><strong>${monthStats.lowest??"—"}点</strong></div><div><small>今月改善</small><strong>${monthStats.improvement===null?"—":`${monthStats.improvement>=0?"+":""}${monthStats.improvement}点`}</strong></div></div>`}}
