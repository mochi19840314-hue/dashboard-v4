...
const base={entries:[],learningHistory:[],weeklyLearningHistory:[],successLibrary:[],strategyMap:{},seasonLearning:[],forecastModel:{},coachHistory:[],settings:{},weatherCache:null,meta:{lastUpdated:null},finance:{balance:0,monthlyExpense:0,personnelExpense:0,medicalExpense:0,cardFee:0,loan:0,repayment:0,incomeTarget:0,morikuboOnline:0,royalCanin:0,purina:0},financeByMonth:{},monthlyReports:{},historical:{...HISTORICAL},clinic:{...DEFAULT_CLINIC},memo:""};
...
function renderManagementCompass(result){
 ...
 content.innerHTML=`<div class="compass-priority"><span>${"★".repeat(result.impact)}${"☆".repeat(5-result.impact)}</span><small>今日の最優先</small><h3>${escapeHtml(result.title)}</h3></div><section><h4>Mission</h4><ul>${mission.actions.slice(0,3).map(action=>`<li>${escapeHtml(action)}</li>`).join("")}</ul></section><div class="compass-values"><section><h4>追加期待売上</h4><strong>${revenue}</strong></section><section><h4>期待利益</h4><strong>${profit}</strong></section><section><h4>実績傾向</h4><strong>${escapeHtml(result.performanceTrend)}</strong></section></div><section class="compass-reason"><h4>理由</h4><p>${escapeHtml(result.reason)}</p></section>${result.next?`<section class="compass-next"><h4>次点</h4><strong>${escapeHtml(result.next.title)}　追加期待売上 約${yen(result.next.expectedIncrementalSales)}　期待利益 約${yen(result.next.expectedProfit)}</strong></section>`:""}`;
}
...
function renderStrategyIntelligence(){
 ...
 $("aiManagementBrief").innerHTML=[["今日の最優先",brief.priority],["今日のリスク",brief.risk],["追加期待利益",`約${yen(brief.expectedProfit)}`],["追加期待売上",`約${yen(brief.expectedSales)}`],["今日意識する診療",brief.clinical],["AIコメント",brief.comment],["AIコーチ",result.coach.comment]].map(([label,value],index)=>`<div class="${index>4?"strategy-wide":""}"><span>${label}</span><strong>${escapeHtml(value)}</strong></div>`).join("");
...
function render(){
 const sections=[recent,month,renderMonthlyReport,renderStrategyIntelligence,renderClinicalIntelligence,renderClinicalTrends,years,year,finance,renderClinicSettings,storage,renderTodaySummary,renderDailyShadowBrief,renderTodayStrategy,renderLearningInsight,renderWeeklyInsights,renderSuccessLibrary,renderBusinessAnomalies,renderDailyReview,renderKagemusha,renderKagemushaDiary,renderPhase1Director,renderManagementInsight];
