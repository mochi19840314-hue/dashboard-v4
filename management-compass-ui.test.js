...
test("コンパスは最優先・Mission・理由・次点に集約",()=>{for(const label of["今日の最優先","明日の候補","Mission","理由","次点"])assert.match(render,new RegExp(label));assert.doesNotMatch(render,/実績傾向|performanceTrend/)});
test("必要営業日未満だけ学習中と判定する",()=>{
 ...
 for(const businessDays of[15,40]){const output=renderResult({ready:false,sampleDays:businessDays,requiredDays:15,missions:[]});assert.equal(output.status,"分析中");assert.equal(output.content,'<p class="compass-empty">現在も診療データを分析中です。</p>');assert.doesNotMatch(output.content,/学習中/)}
});
