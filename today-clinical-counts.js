(function(root,factory){
 ...
 const value=(entry,metric)=>{
  ...
  if(metric==="surgeries"||metric==="trimming"||metric==="checkups"||metric==="secondOpinions")return number(entry?.[metric]);
  return number(clinical[metric]);
 };
 function update(entries,date,metric,delta){
  if(!Array.isArray(entries)||!/^\d{4}-\d{2}-\d{2}$/.test(String(date))||!["preventive","checkups","imaging","bloodTests","surgeries","trimming","secondOpinions"].includes(metric))return null;
  ...
  }else if(metric==="surgeries"||metric==="trimming"||metric==="checkups"||metric==="secondOpinions")entry[metric]=next;
