(function(root,factory){const api=factory();if(typeof module!=="undefined"&&module.exports)module.exports=api;else root.CashFlowForecast=api})(typeof globalThis!=="undefined"?globalThis:this,function(){
 ...
 const PAYMENT_DAYS={rent:10,medical:10,card:15,tax:15,salary:27,lease:27,other:27};
 ...
 function calculate({balance=0,currentSales=0,projectedSales=0,uncollected=0,otherIncome=0,cardReceipts={},today=new Date(),payments={},previousBalance=null,currentProfit=0,previousMedicalExpense=0,dayOfMonth}={}){
  ...
  Object.entries(scheduled).forEach(([key,value])=>{const day=Number(payments[`${key}Day`]??PAYMENT_DAYS[key]);if(day>=currentDay)events.push({day,incoming:0,outgoing:value})});
  ...
  const incoming=receipts.card+receipts.uncollected+receipts.otherIncome,outgoing=daily.reduce((sum,event)=>sum+event.outgoing,0),forecastBalance=runningBalance,rating=safety(minimumBalance);
  ...
  if(cards.missing.length)comment="カード入金予定額が未入力です。";
  ...
  return{balance:present,receipts,cards,scheduled,incoming,outgoing,minimumBalance,forecastBalance,timeline:daily,safety:rating,confidence:cards.confidence,monthOverMonth,comment};
 }
 return{calculate,cardSchedule,safety,PAYMENT_DAYS};
});
