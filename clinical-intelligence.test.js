...
const html=fs.readFileSync("index.html","utf8"),sw=fs.readFileSync("sw.js","utf8"),app=fs.readFileSync("app.js","utf8");assert.match(html,/診療構成インサイト/);assert.match(html,/clinical-intelligence\.js\?v=1079/);assert.match(sw,/clinical-intelligence\.js\?v=1079/);assert.match(sw,/keita-dashboard-v1080-annual-profit-forecast/);assert.match(app,/function renderClinicalIntelligence\(\)[\s\S]*?catch\(error\)\{console\.error\(error\)\}/,"render failure is isolated");assert.doesNotMatch(app,/localStorage[^\n]*clinicalAnalysis|clinicalAnalysis[^\n]*localStorage/,"analysis isn't persisted");
